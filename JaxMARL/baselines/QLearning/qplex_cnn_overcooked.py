"""QPLEX-style CNN baseline for Overcooked v2.

This keeps the local CNN per-agent Q-network but replaces the joint mixer with
a duplex-dueling form:

    Q_tot(s, u) = V_tot(s) + sum_i lambda_i(s) * A_i(o_i, u_i)

where ``V_tot`` is a monotonic mixer over each agent's max action value and
``A_i`` is the chosen value minus that max value. The deployed partner policy
is still the greedy per-agent Q policy, so it plugs into the existing 1-ZSC
loader cleanly.
"""
import os
import copy
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                q_next_target_all = jax.vmap(network.apply, in_axes=(None, 0))(
                    train_state.target_network_params["agent"],
                    batchify(minibatch.second.obs),
                )  # (num_agents, batch_size, ...)
                q_next_target = jnp.max(q_next_target_all, axis=-1)
                ...
                def _loss_fn(params):
                    ...
                    max_agent_q_vals = jnp.max(q_vals, axis=-1)
                    ...
                    loss = jnp.mean(
                        (chosen_qmix - jax.lax.stop_gradient(qmix_target)) ** 2
                    )
