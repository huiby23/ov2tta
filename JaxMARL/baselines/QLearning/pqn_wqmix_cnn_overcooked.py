"""PQN-VDN with WQMIX-style optimistic weighted TD loss for OV2."""
import os
import copy
import jax
            ...
            def _learn_epoch(carry, _):
                ...
                def _learn_phase(carry, minibatch_and_target):
                    ...
                    def _loss_fn(params):
                        ...
                        q_detached = jax.lax.stop_gradient(vdn_chosen_action_qvals)
                        td_error = vdn_chosen_action_qvals - target_detached
                        wqmix_alpha = float(config.get("WQMIX_ALPHA", 0.5))
                        weights = jnp.where(target_detached > q_detached, 1.0, wqmix_alpha)
...
def single_run(config):
    ...
    alg_name = config.get('ALG_NAME', "pqn_wqmix_cnn")
