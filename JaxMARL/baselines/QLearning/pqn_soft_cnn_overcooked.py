"""PQN-VDN with SHAQ/soft-Q style soft bootstrap targets for OV2."""
import os
import copy
import jax
...
def make_train(config, env):
    ...
    def _soft_agent_values(q_values):
        tau = float(config.get("SOFT_Q_TEMPERATURE", 0.25))
        if tau <= 0.0:
            return jnp.max(q_values, axis=-1)
        soft_values = tau * jax.nn.logsumexp(q_values / tau, axis=-1)
        if bool(config.get("SOFT_Q_CENTER", True)):
            action_count = jnp.asarray(q_values.shape[-1], dtype=q_values.dtype)
            soft_values = soft_values - tau * jnp.log(action_count)
        return soft_values

    def _joint_next_values(q_values):
        return jnp.sum(_soft_agent_values(q_values), axis=0)
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            last_q = _joint_next_values(last_q)  # (num_envs) soft VDN bootstrap
            ...
            def _compute_targets(last_q, q_vals, reward, done):
                def _get_target(lambda_returns_and_next_q, rew_q_done):
                    ...
                    next_q = _joint_next_values(q)  # sum soft values over agents
                    return (lambda_returns, next_q), lambda_returns

                lambda_returns = reward[-1] + config["GAMMA"] * (1 - done[-1]) * last_q
                last_q = _joint_next_values(q_vals[-1])  # sum soft values over agents
...
def single_run(config):
    ...
    alg_name = config.get('ALG_NAME', "pqn_soft_cnn")
