"""Weighted QMIX-style CNN baseline for Overcooked v2.

The agent network and centralized monotonic mixer match the local QMIX-CNN
implementation. The update uses a weighted TD objective: transitions whose
bootstrapped target is above the current joint Q receive full weight, while
the remaining TD errors receive ``WQMIX_ALPHA``. This is the lightweight
optimistic weighting variant that is cheap to run in the existing OV2 queue.
"""
import os
import copy
import jax
...
def make_train(config, env):
    ...
    config["MIXER_RESIDUAL_SUM"] = bool(config.get("MIXER_RESIDUAL_SUM", True))
    config["WQMIX_ALPHA"] = float(config.get("WQMIX_ALPHA", 0.1))
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            agent_params = network.init(rng_agent, init_x)
            state_size = int(np.prod(env.observation_space().shape)) * len(env.agents)
            mixer_params = mixer.init(
                rng_mixer,
                jnp.zeros((len(env.agents), 1)),
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                if config["MIXER_RESIDUAL_SUM"]:
                    qmix_next = qmix_next + jnp.sum(q_next_target, axis=0)

                qmix_target = minibatch.first.rewards["__all__"] + (
                    1 - minibatch.first.dones["__all__"]
                ) * config["GAMMA"] * qmix_next
                ...
                def _loss_fn(params):
                    ...
                    # get Q-values of the chosen actions for each agent
                    chosen_agent_q_vals = jnp.take_along_axis(
                        q_vals,
                        batchify(minibatch.first.actions)[..., jnp.newaxis],
                        axis=-1,
                    ).squeeze()  # (num_agents, batch_size, )

                    chosen_qmix = mixer.apply(
                        params["mixer"],
                        chosen_agent_q_vals,
                        minibatch.first.obs["__all__"],
                    )
                    if config["MIXER_RESIDUAL_SUM"]:
                        chosen_qmix = chosen_qmix + jnp.sum(
                            chosen_agent_q_vals,
                            axis=0,
                        )
                    td_error = chosen_qmix - jax.lax.stop_gradient(qmix_target)
                    weights = jnp.where(
                        jax.lax.stop_gradient(qmix_target) > chosen_qmix,
                        1.0,
                        config["WQMIX_ALPHA"],
                    )
                    loss = jnp.mean(weights * td_error**2)

                    return loss, chosen_qmix.mean()
