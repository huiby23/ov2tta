...
REPORT_DIR="${REPORT_DIR:-reports/ttac_v5_4_sharded_full_eval_${TS}}"
LOG_DIR="${LOG_DIR:-logs/ttac_v5_4_sharded_full_eval_${TS}}"

MODE="${MODE:?Set MODE, e.g. ttac_v5_4_delta}"
OUTPUT_TAG="${OUTPUT_TAG:-v5_4_full_${MODE}_${TS}}"
SEED="${SEED:-42}"
EVAL_NUM_SEEDS="${EVAL_NUM_SEEDS:-500}"
SEED_CHUNKS="${SEED_CHUNKS:-1}"
...
run_shard() {
  ...
  "${PYTHON}" experiments/overcooked_v2_experiments/ttac_v5_5_delta_push/utils/visualize_ppo.py \
    --d "${RUN_DIR}" \
    --seed "${chunk_seed}" \
...
md = [
    "# Sharded TTAC v5.4 full eval summary",
    "",
    "| metric | value |",
    "|---|---:|",
    ...
    f"| seed_chunks | {summary['seed_chunks']} |",
    "",
