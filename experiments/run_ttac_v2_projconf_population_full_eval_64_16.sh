...
TS="${TS:-$(date +%Y%m%d_%H%M%S_projconf_pop)}"
...
LOG_DIR="${LOG_DIR:-logs/ttac_v2_projconf_population_full_eval_${TS}}"
REPORT_DIR="${REPORT_DIR:-reports/ttac_v2_projconf_population_full_eval_${TS}}"
...
run_train() {
  ...
  local prefix="ttac_v2_projconf_pop_${phase}_state_aug_64_16_10M_seed42_10seeds_${TS}"
  local log_file="${LOG_DIR}/train_${phase}.log"
  log "TRAIN_START phase=${phase} prefix=${prefix} surrogate=projected_confident population=${POPULATION_RUN_DIR}"
  "${PYTHON}" -m overcooked_v2_experiments.ttac_v2.main \
    ...
    model.TTAC_TRAIN_SURROGATE=projected_confident model.TTAC_TRAIN_PROJECT_BETA="${TTAC_TRAIN_PROJECT_BETA}" \
...
if len(out):
    ...
    (report / "full_eval_summary.md").write_text(
        "# TTACv2 projected-confident + frozen-population full eval\n\n"
        f"run_dir: `{run_dir}`\n\n"
        + table
        + "\n"
    )
