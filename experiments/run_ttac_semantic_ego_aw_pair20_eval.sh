...
for mode in "${MODES[@]}"; do
  ...
  "$PYTHON" experiments/overcooked_v2_experiments/ttac_v2/utils/visualize_ppo.py \
    ...
    --no_viz \
    --ttac_mode "$mode" \
    --output_tag "semantic_pair20_100_${mode}" \
    --eval_batches 10 \
    --max_pairings 20 \
    ...
    --ttac_test_contrast_beta 1.0 \
    --ttac_test_contrast_floor 0.0 \
    --ttac_test_semantic_lambda 0.25
  echo "EVAL_DONE ${mode}"
done
...
rows = []
for path in sorted(run.glob("reward_summary_cross_semantic_pair20_100_*.csv")):
    mode = path.name.removeprefix("reward_summary_cross_semantic_pair20_100_").removesuffix(".csv")
...
out = run / "ttac_semantic_ego_aw_pair20_100_summary.csv"
