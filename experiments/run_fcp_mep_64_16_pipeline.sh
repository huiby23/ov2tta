...
PREFIX="${PREFIX:-fcp_mep_coplay_K${POPULATION_SIZE:-5}_${NUM_ENVS:-64}_${NUM_MINIBATCHES:-16}_${TOTAL_TIMESTEPS:-10000000}_${JOB_TAG}}"
LOG_DIR="${LOG_DIR:-logs/fcp_mep}"
mkdir -p "${LOG_DIR}"
...
print(f"[fcp-mep] result SP={statistics.mean(sp) if sp else math.nan:.4f} XP={statistics.mean(xp) if xp else math.nan:.4f} n_sp={len(sp)} n_xp={len(xp)} csv={path}")
PY
}

run_pipeline() {
  echo "[fcp-mep] train_start=$(date -Is) prefix=${PREFIX} log=${LOG}"
  PREFIX="${PREFIX}" bash experiments/run_fcp_mep_64_16.sh
  local run_dir
  run_dir="$(latest_run_dir)"
  if [[ -z "${run_dir}" || ! -d "${run_dir}/run_0/ckpt_final" ]]; then
    echo "[fcp-mep] missing run_dir/checkpoint prefix=${PREFIX} run_dir=${run_dir}" >&2
    exit 1
  fi
  echo "[fcp-mep] train_done=$(date -Is) run_dir=${run_dir}"
  ...
  echo "[fcp-mep] eval_done=$(date -Is) run_dir=${run_dir}"
}

if [[ "${1:-}" == "status" ]]; then
  pgrep -af "${PREFIX}|overcooked_v2_experiments.fcp_mep|visualize_ppo" || true
