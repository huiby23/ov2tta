...
for mode in "${MODES[@]}"; do
  ...
  "$PYTHON" experiments/overcooked_v2_experiments/ttac_v2/utils/visualize_ppo.py \
    ...
    --no_viz \
    --ttac_mode "$mode" \
    --output_tag "contrast_pair20_100_${mode}" \
    --eval_batches 10 \
    --max_pairings 20 \
...
rows = []
for path in sorted(run.glob("reward_summary_cross_contrast_pair20_100_*.csv")):
    mode = path.name.removeprefix("reward_summary_cross_contrast_pair20_100_").removesuffix(".csv")
...
out = run / "ttac_contrastive_ego_aw_pair20_100_summary.csv"
