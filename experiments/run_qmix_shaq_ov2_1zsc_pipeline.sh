...
RUN_SHAQ="${RUN_SHAQ:-1}"
RUN_EVAL="${RUN_EVAL:-1}"
SHAQ_BUFFER_SIZE="${SHAQ_BUFFER_SIZE:-20000}"
SHAQ_BUFFER_BATCH_SIZE="${SHAQ_BUFFER_BATCH_SIZE:-64}"
...
run_shaq=${RUN_SHAQ}
run_eval=${RUN_EVAL}
shaq_buffer_size=${SHAQ_BUFFER_SIZE}
shaq_buffer_batch_size=${SHAQ_BUFFER_BATCH_SIZE}
...
if [[ "${RUN_SHAQ}" == "1" ]]; then
  log "START SHAQ OV2 layout=${LAYOUT} seed=${SEED} split_seeds=${NUM_SEEDS} total=${TOTAL_TIMESTEPS} buffer=${SHAQ_BUFFER_SIZE}/${SHAQ_BUFFER_BATCH_SIZE}"
  for i in $(seq 0 $((NUM_SEEDS - 1))); do
    ...
    "${PYTHON}" experiments/overcooked_v2_experiments/qlearning/utils/train_shaq_ov2.py \
      ...
      --num_envs "${NUM_ENVS}" \
      --num_steps "${NUM_STEPS}" \
      --buffer_size "${SHAQ_BUFFER_SIZE}" \
      --buffer_batch_size "${SHAQ_BUFFER_BATCH_SIZE}" \
