...
JOB_ROOT="${JOB_ROOT:-logs/e3t_state_aug_official_ppo_hparams_ce8}"
...
status() {
  if is_running; then
    local pid
    pid="$(cat "${PID_FILE}")"
    echo "[e3t-ppo-ce8] running pid=${pid}"
    ps -fp "${pid}" || true
    echo "[e3t-ppo-ce8] children:"
    pgrep -P "${pid}" -af || true
  else
    echo "[e3t-ppo-ce8] not running"
    if [[ -f "${PID_FILE}" ]]; then
      echo "[e3t-ppo-ce8] stale pid=$(cat "${PID_FILE}")"
    fi
  fi
  ...
  if [[ -f "${LATEST_LOG}" ]]; then
    local log
    log="$(cat "${LATEST_LOG}")"
    echo "[e3t-ppo-ce8] latest_log=${log}"
    if [[ -f "${log}" ]]; then
      echo "[e3t-ppo-ce8] log_tail:"
      tail -n 60 "${log}"
    fi
  fi
}
...
run_train() {
  ...
  echo "[e3t-ppo-ce8] stage_start=$(date -Is) prefix=${prefix} rand=${rand}"
  "${PYTHON}" experiments/overcooked_v2_experiments/e3t_state_aug_official/main.py \
    ...
    model.RAND="${rand}"
  echo "[e3t-ppo-ce8] stage_done=$(date -Is) prefix=${prefix}"
}
...
pipeline() {
  ...
  echo "[e3t-ppo-ce8] launch_time=$(date -Is)"
  echo "[e3t-ppo-ce8] host=$(hostname)"
  echo "[e3t-ppo-ce8] root=${ROOT}"
  echo "[e3t-ppo-ce8] python=${PYTHON}"
  echo "[e3t-ppo-ce8] layout=${LAYOUT} seed=${SEED} num_seeds=${NUM_SEEDS}"
  echo "[e3t-ppo-ce8] wandb_entity=${WANDB_ENTITY} wandb_project=${WANDB_PROJECT} wandb_mode=${WANDB_MODE}"
  echo "[e3t-ppo-ce8] cuda_visible_devices=${CUDA_VISIBLE_DEVICES:-unset}"

  # Stage B: same E3T structure, but PPO-CNN baseline optimization/budget.
  for rand_cfg in 0.0 0.3 0.7; do
    rand_tag="${rand_cfg/./}"
    run_train \
      "figure4_e3t_ppo_hparams_ce8_rand${rand_tag}_${JOB_TAG}" \
      "1e7" "5e6" "256" "64" "4" "16" \
      "4e-4" "0.95" "0.5" "0.5" "True" "0.05" "0.2" "0.04" "${rand_cfg}"
  done

  echo "[e3t-ppo-ce8] pipeline_done=$(date -Is)"
}
...
start() {
  ...
  local pid=$!
  echo "${pid}" > "${PID_FILE}"
  echo "[e3t-ppo-ce8] started pid=${pid} log=${log}"
}
...
stop() {
  if is_running; then
    ...
    pid="$(cat "${PID_FILE}")"
    kill "${pid}"
    echo "[e3t-ppo-ce8] sent TERM to pid=${pid}"
  else
    echo "[e3t-ppo-ce8] not running"
  fi
}
...
case "${1:-start}" in
  ...
  tail)
    if [[ ! -f "${LATEST_LOG}" ]]; then
      echo "[e3t-ppo-ce8] no latest log" >&2
      exit 1
    fi
