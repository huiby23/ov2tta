...
JOB_ROOT="${JOB_ROOT:-logs/ppo_e3t_state_aug_ablation}"
...
status() {
  if is_running; then
    local pid
    pid="$(cat "${PID_FILE}")"
    echo "[ppo-e3t-sa] running pid=${pid}"
    ps -fp "${pid}" || true
    echo "[ppo-e3t-sa] children:"
    pgrep -P "${pid}" -af || true
  else
    echo "[ppo-e3t-sa] not running"
    [[ -f "${PID_FILE}" ]] && echo "[ppo-e3t-sa] stale pid=$(cat "${PID_FILE}")"
  fi
  if [[ -f "${LATEST_LOG}" ]]; then
    local log
    log="$(cat "${LATEST_LOG}")"
    echo "[ppo-e3t-sa] latest_log=${log}"
...
set_scale() {
  if [[ "${MODE}" == "smoke" ]]; then
    ...
    echo "[ppo-e3t-sa] unknown MODE=${MODE}; use smoke or full" >&2
    exit 2
  fi
}
...
run_train() {
  ...
  echo "[ppo-e3t-sa] stage_start=$(date -Is) stage=${stage} actor_condition=${actor_condition} enable_ce=${enable_ce} condition_actor=${condition_actor} num_iterations=${NUM_ITERATIONS}"
  "${PYTHON}" experiments/overcooked_v2_experiments/ppo_e3t_official/main.py \
    ...
    model.RAND=0.0 \
    model.COPY=0.0
  echo "[ppo-e3t-sa] stage_done=$(date -Is) stage=${stage}"
}

run_stage() {
  case "$1" in
    predicted_ce) run_train "predicted_ce" "predicted_partner" "True" "True" ;;
    constant_ce) run_train "constant_ce" "constant" "True" "True" ;;
    no_ce) run_train "no_ce" "predicted_partner" "False" "True" ;;
    *) echo "[ppo-e3t-sa] unknown stage=$1" >&2; exit 2 ;;
  esac
}
...
pipeline() {
  ...
  print_repro_env
  set_scale
  "${PYTHON}" experiments/tools/seed_manifest.py --seed "${SEED}" --num-seeds "${NUM_SEEDS}" --num-iterations "${NUM_ITERATIONS}" --out "${JOB_ROOT}/seed_manifest_${JOB_TAG}.json" || true
  echo "[ppo-e3t-sa] launch_time=$(date -Is) host=$(hostname) root=${ROOT} cuda=${CUDA_VISIBLE_DEVICES:-unset}"
  echo "[ppo-e3t-sa] mode=${MODE} layout=${LAYOUT} seed=${SEED} num_seeds=${NUM_SEEDS} stages=${STAGES} num_iterations=${NUM_ITERATIONS}"
  echo "[ppo-e3t-sa] scale total=${TOTAL_TIMESTEPS} envs=${NUM_ENVS} steps=${NUM_STEPS} epochs=${UPDATE_EPOCHS} ce_epochs=${CONTEXT_UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES}"
  for stage in ${STAGES}; do
    run_stage "${stage}"
  done
  echo "[ppo-e3t-sa] pipeline_done=$(date -Is)"
  rm -f "${PID_FILE}"
}
...
start() {
  ...
  export PREFIX_BASE="${PREFIX_BASE:-ppo_e3t_state_aug}"
  ...
  local log="${JOB_ROOT}/ppo_e3t_state_aug_${JOB_TAG}.log"
  printf "%s\n" "${log}" > "${LATEST_LOG}"
  nohup env JOB_TAG="${JOB_TAG}" MODE="${MODE}" PREFIX_BASE="${PREFIX_BASE}" LAYOUT="${LAYOUT}" \
    ...
    WANDB_ENTITY="${WANDB_ENTITY}" CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" PYTHON="${PYTHON}" NUM_ITERATIONS="${NUM_ITERATIONS:-}" \
    bash "${BASH_SOURCE[0]}" pipeline > "${log}" 2>&1 < /dev/null &
  local pid=$!
  echo "${pid}" > "${PID_FILE}"
  echo "[ppo-e3t-sa] started pid=${pid} log=${log}"
}

case "${1:-start}" in
  start) start ;;
  pipeline)
    : "${MODE:=full}" "${PREFIX_BASE:=ppo_e3t_state_aug}" "${LAYOUT:=counter_circuit}" "${SEED:=42}"
  ...
  stop)
    if is_running; then kill "$(cat "${PID_FILE}")"; else echo "[ppo-e3t-sa] not running"; fi ;;
