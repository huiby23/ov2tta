...
class TTACPolicyState:
    params: core.FrozenDict[str, Any]
    online_decoder_params: core.FrozenDict[str, Any]
    base_hstate: Any
...
EVAL_MODES = (
    ...
    "ttac_v5_8_latent_decoder_blend_online_lora_wrong_history",
    "ttac_v5_8_latent_decoder_blend_online_lora_random_history",
    "ttac_v5_8_latent_decoder_blend_online_lora_delayed_history",
...
def _is_trainable_lora_key(key: str) -> bool:
    return (
        key in ("lora_a", "lora_b", "lora_bias", "lora_gate_w", "lora_gate_b")
        or key.startswith("lora_coeff_")
    )


def _mask_lora_decoder_grads(grads):
    flat = flatten_dict(grads, sep="/")
    masked = {
        key: (value if _is_trainable_lora_key(key.split("/")[0]) else jnp.zeros_like(value))
        for key, value in flat.items()
    }
    return unflatten_dict(masked, sep="/")


def _lora_l2_to_init(params, init_params):
    params_flat = flatten_dict(params, sep="/")
    init_flat = flatten_dict(init_params, sep="/")
    terms = []
    for key, value in params_flat.items():
        if _is_trainable_lora_key(key.split("/")[0]) and key in init_flat:
            terms.append(jnp.mean(jnp.square(value - jax.lax.stop_gradient(init_flat[key]))))
    if not terms:
        return jnp.asarray(0.0, dtype=jnp.float32)
    return jnp.mean(jnp.stack(terms))


def _tree_where(condition, true_tree, false_tree):
    return jax.tree_util.tree_map(
        lambda t, f: jnp.where(condition, t, f),
        true_tree,
        false_tree,
    )
...
class PPOPolicy(AbstractPolicy):
    ...
    def _is_latent_decoder_online_lora_mode(self):
        return (
            self.eval_mode.startswith("ttac_v5_8_latent_decoder_blend_online_lora")
            or "latent_decoder_online_lora" in self.eval_mode
        )
    ...
    def _initial_online_decoder_params(self):
        decoder_params = self.model_config.get("TTAC_LATENT_DECODER", None)
        if decoder_params is None:
            return core.freeze({})
        if isinstance(decoder_params, (list, tuple)):
            return decoder_params[0]
        return decoder_params

    def _reset_online_decoder_params(self, hstate, done):
        init_params = self._initial_online_decoder_params()
        if not init_params:
            return hstate.online_decoder_params
        return _tree_where(jnp.asarray(done), init_params, hstate.online_decoder_params)
    ...
    def _target_direct_blend(self, pi, obs, hstate, done):
        ...
        if self._is_latent_decoder_mode():
            ...
            if self._is_latent_decoder_online_z_mode() or self._is_latent_decoder_online_lora_mode():
    ...
    def _ttac_latent_decoder_member_probs(self, context, hstate, forced_history_mode=None):
        ...
        if self._is_latent_decoder_online_lora_mode() and forced_history_mode is None:
            decoder_logits = apply_latent_partner_decoder(
                hstate.online_decoder_params,
                selected_obs[None, ...],
                chron_obs[None, ...],
                chron_actions[None, :],
                self.action_dim,
            ...
            return jax.lax.stop_gradient(
                jax.nn.softmax(decoder_logits, axis=-1)[None, ...]
            )
    ...
    def _ttac_latent_decoder_online_z_update(self, hstate):
        ...
        chron_obs = jnp.where(obs_valid, chron_obs, jnp.zeros_like(chron_obs))
        chron_actions = jnp.where(valid, chron_actions, jnp.zeros_like(chron_actions))
        support_fraction = jnp.asarray(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_SUPPORT_FRACTION", 1.0),
            dtype=jnp.float32,
        )
        support_fraction = jnp.clip(support_fraction, 0.0, 1.0)
        valid_rank = jnp.cumsum(valid.astype(jnp.int32))
        total_valid = jnp.sum(valid.astype(jnp.int32))
        support_count = jnp.maximum(
            jnp.array(1, dtype=jnp.int32),
            jnp.asarray(
                jnp.round(total_valid.astype(jnp.float32) * support_fraction),
                dtype=jnp.int32,
            ),
        )
        support_count = jnp.minimum(support_count, jnp.maximum(total_valid, 1))
        adapt_valid = valid & (valid_rank <= support_count)
        adapt_obs_valid = adapt_valid.reshape((self.history_len,) + (1,) * len(self.obs_shape))
        adapt_obs = jnp.where(adapt_obs_valid, chron_obs, jnp.zeros_like(chron_obs))
        adapt_actions = jnp.where(adapt_valid, chron_actions, jnp.zeros_like(chron_actions))

        history_mode = str(self.model_config.get("TTAC_LATENT_DECODER_HISTORY_MODE", "full"))
        encoder_actions = adapt_actions
        if history_mode == "action_only":
            encoder_obs = jnp.zeros_like(adapt_obs)
        elif history_mode == "no_history":
            encoder_obs = jnp.zeros_like(adapt_obs)
            encoder_actions = jnp.zeros_like(adapt_actions)
        else:
            encoder_obs = adapt_obs
        ...
        valid_f = adapt_valid.astype(jnp.float32)
        ...
        online_residual_c = "latent_online_residual_c_enabled" in main_decoder_params
        if online_residual_c:
            base_z_dim = int(main_decoder_params["vae_mu_b"].shape[0])
            base_part = base_z[:base_z_dim]
            prev_c = hstate.latent_z[base_z_dim:]
            zero_c = base_z[base_z_dim:]
            start_c = jnp.where(hstate.count <= min_history, zero_c, prev_c)
            start_z = jnp.concatenate([base_part, start_c], axis=0)
        smooth_coef = jnp.asarray(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_SMOOTH_COEF", 0.0),
            dtype=jnp.float32,
        )

        def _canonical_z(z):
            if online_residual_c:
                return jnp.concatenate([base_part, z[base_z_dim:]], axis=0)
            return z
        ...
        def _loss(z):
            ...
            if online_residual_c:
                ...
                smooth = jnp.mean(jnp.square(c - jax.lax.stop_gradient(start_c)))
            else:
                prior = jnp.mean(jnp.square(z - jax.lax.stop_gradient(base_z)))
                smooth = jnp.asarray(0.0, dtype=jnp.float32)
            return ce + prior_coef * prior + smooth_coef * smooth

        def _one_step(z, _):
            grad = jax.grad(_loss)(z)
            next_z = z - jnp.asarray(lr, dtype=jnp.float32) * grad
            next_z = jnp.clip(next_z, -z_clip, z_clip)
            next_z = _canonical_z(next_z)
            return next_z, None
    ...
    def _ttac_latent_decoder_online_lora_update(self, hstate):
        init_params = self._initial_online_decoder_params()
        if not init_params:
            return hstate
        if "latent_lora_residual_enabled" not in init_params:
            return hstate
        ...
        val_steps = int(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_VAL_STEPS", 10)
        )
        val_steps = max(1, min(self.history_len - 1, val_steps))
        pos = jnp.arange(self.history_len)
        val_mask = valid & (pos >= (self.history_len - val_steps))
        support_mask = valid & (~val_mask)
        support_f = support_mask.astype(jnp.float32)
        val_f = val_mask.astype(jnp.float32)
        support_denom = jnp.maximum(jnp.sum(support_f), 1.0)
        val_denom = jnp.maximum(jnp.sum(val_f), 1.0)

        init_logits = apply_latent_partner_decoder_with_z(
            init_params,
            chron_obs,
            z_batch,
            self.action_dim,
        )
        init_probs = jax.lax.stop_gradient(jax.nn.softmax(init_logits, axis=-1))
        init_log_probs = jax.lax.stop_gradient(jax.nn.log_softmax(init_logits, axis=-1))

        def _trajectory_nll(params, mask_f, denom):
            logits = apply_latent_partner_decoder_with_z(
                params,
                chron_obs,
                z_batch,
            ...
            return -jnp.sum(action_log_probs * mask_f) / denom

        anchor_coef = jnp.asarray(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_ANCHOR_KL_COEF", 0.05),
            dtype=jnp.float32,
        )
        l2_coef = jnp.asarray(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_L2_COEF", 0.001),
            dtype=jnp.float32,
        )
        lr = float(self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_LR", 0.001))
        steps = int(self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_UPDATE_STEPS", 5))
        ...
        def _loss(params):
            ...
            ce = -jnp.sum(action_log_probs * support_f) / support_denom
            anchor_kl = jnp.sum(
                init_probs * (init_log_probs - log_probs),
                axis=-1,
            )
            anchor_kl = jnp.sum(anchor_kl * support_f) / support_denom
            l2 = _lora_l2_to_init(params, init_params)
            return ce + anchor_coef * anchor_kl + l2_coef * l2

        def _one_step(params, _):
            grads = jax.grad(_loss)(params)
            grads = _mask_lora_decoder_grads(grads)
            next_params = jax.tree_util.tree_map(
                lambda p, g: p - jnp.asarray(lr, dtype=jnp.float32) * g,
                params,
                grads,
            )
            return next_params, None

        start_params = hstate.online_decoder_params
        before_val_nll = _trajectory_nll(start_params, val_f, val_denom)
        if steps <= 0:
            candidate_params = start_params
        else:
            candidate_params, _ = jax.lax.scan(_one_step, start_params, None, length=steps)
        after_val_nll = _trajectory_nll(candidate_params, val_f, val_denom)

        accept_margin = jnp.asarray(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_ACCEPT_MARGIN", 0.0),
            dtype=jnp.float32,
        )
        accept_validation = bool(
            self.model_config.get("TTAC_LATENT_DECODER_ONLINE_LORA_ACCEPT_VALIDATION", True)
        )
        enough_support = jnp.sum(support_f) > 0.0
        enough_val = jnp.sum(val_f) > 0.0
        accept = enough_support & enough_val & (after_val_nll < before_val_nll - accept_margin)
        if not accept_validation:
            accept = enough_support
        next_params = _tree_where(accept, candidate_params, start_params)
        return hstate.replace(online_decoder_params=next_params)
    ...
    def update_after_step(self, hstate, partner_obs, partner_action, done):
        ...
        if self._is_direct_blend_mode():
            ...
            if self._is_latent_decoder_online_lora_mode():
                update_interval = int(
                    self.model_config.get(
                        "TTAC_LATENT_DECODER_ONLINE_UPDATE_INTERVAL", 10
                    )
                ...
                update_interval = max(update_interval, 1)
                min_history = jnp.asarray(
                    self.model_config.get("TTAC_LATENT_DECODER_ONLINE_MIN_HISTORY", 20),
                    dtype=jnp.int32,
                )
                interval_gate = (hstate.global_step % update_interval) == 0
                should_update_lora = (
                ...
                hstate = jax.lax.cond(
                    should_update_lora,
                    self._ttac_latent_decoder_online_lora_update,
                    lambda state: state,
                    operand=hstate,
                )
            valid_mask = jnp.where(done, jnp.zeros_like(hstate.valid_mask), hstate.valid_mask)
            return hstate.replace(
                ...
                online_decoder_params=self._reset_online_decoder_params(hstate, done),
                prev_partner_action=true_partner_action,
                prev_memory_action=jnp.where(
                    done, jnp.array(0, dtype=jnp.int32), hstate.prev_memory_action
                ),
    ...
    def init_hstate(self, batch_size, key=None):
        ...
        return TTACPolicyState(
            params=self.params,
            online_decoder_params=self._initial_online_decoder_params(),
...
def _functional_init_ttac_state(spec, config, obs_shape, history_len):
    ...
    return TTACPolicyState(
        params=spec.params,
        online_decoder_params=core.freeze({}),
