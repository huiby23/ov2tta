...
def build_query_groups(arrays, pair_labels, indices, scope):
    if scope == "all" or pair_labels is None:
        return {"all": np.asarray(indices, dtype=np.int64)}, lambda _idx: "all"

    labels = np.asarray(pair_labels).astype(str)
    roles = np.asarray(arrays.get("role", np.zeros(len(labels), dtype=np.int8)))

    def key_fn(idx):
        if scope == "pair":
            return labels[int(idx)]
        if scope == "role":
            return f"role={int(roles[int(idx)])}"
        return f"{labels[int(idx)]}|role={int(roles[int(idx)])}"

    groups = {}
    for idx in indices:
        groups.setdefault(key_fn(idx), []).append(int(idx))
    return {k: np.asarray(v, dtype=np.int64) for k, v in groups.items()}, key_fn
...
def main():
    ...
    parser.add_argument("--query_expansion_k", type=int, default=1)
    parser.add_argument("--query_expansion_scope", choices=["pair_role", "pair", "role", "all"], default="pair_role")
    parser.add_argument("--query_expansion_partner_obs", choices=["query", "source"], default="query")
    args = parser.parse_args()
    if args.query_expansion_k < 1:
        raise ValueError("--query_expansion_k must be >= 1")

    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    data = np.load(args.dataset, allow_pickle=False)
    pair_labels = data["pair_label"] if "pair_label" in data.files else None
    ...
    def make_batch(idx, hist_source="partner", expand=False):
        source_idx = np.asarray(idx, dtype=np.int64)
        query_idx = source_idx
        if expand and args.query_expansion_k > 1 and hist_source == "partner":
            expanded_queries = np.empty((len(source_idx), args.query_expansion_k), dtype=np.int64)
            expanded_queries[:, 0] = source_idx
            for row, source in enumerate(source_idx):
                members = query_groups.get(query_group_key(source), source_idx)
                for col in range(1, args.query_expansion_k):
                    expanded_queries[row, col] = rng_np.choice(members)
            query_idx = expanded_queries.reshape(-1)
            source_idx = np.repeat(source_idx, args.query_expansion_k)
        partner_obs_idx = query_idx if args.query_expansion_partner_obs == "query" else source_idx
        return (
            jnp.asarray(arrays["query_obs"][query_idx], dtype=jnp.float32),
            jnp.asarray(arrays["partner_obs"][partner_obs_idx], dtype=jnp.float32),
            jnp.asarray(_history(arrays, source_idx, source=hist_source), dtype=jnp.int32),
            jnp.asarray(arrays["target_partner_probs"][query_idx], dtype=jnp.float32),
        )

    query_groups, query_group_key = build_query_groups(arrays, pair_labels, train_idx, args.query_expansion_scope)
    log_rows = []
    for step in range(1, args.steps + 1):
        batch_idx = rng_np.choice(train_idx, size=args.batch_size, replace=len(train_idx) < args.batch_size)
        params, opt_state, loss, aux = train_step(params, opt_state, make_batch(batch_idx, expand=True))
    ...
    metrics["query_expansion_k"] = int(args.query_expansion_k)
    metrics["query_expansion_groups"] = int(len(query_groups))
    metrics["query_expansion_scope"] = args.query_expansion_scope
    metrics["query_expansion_partner_obs"] = args.query_expansion_partner_obs
