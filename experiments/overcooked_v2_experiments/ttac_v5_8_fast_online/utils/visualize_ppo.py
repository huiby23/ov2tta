...
from overcooked_v2_experiments.ttac_v5_8_fast_online.policy import (
    PPOParams,
...
from overcooked_v2_experiments.ttac_v5_8_fast_online.utils.surrogate_heads import load_surrogate_npz
from overcooked_v2_experiments.ttac_v5_8_fast_online.utils.agreement_heads import load_agreement_npz
from overcooked_v2_experiments.ttac_v5_8_fast_online.utils.latent_partner_decoder import load_latent_decoder_npz
...
def visualize_ppo_policy(
    ...
    if ttac_mode.startswith("ttac_policy_bank"):
        bank_keys = sorted(run_keys, key=lambda x: int(str(x).split("_")[1]))
        ...
        print("TTAC policy bank keys:", bank_keys)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ttac_update_interval", type=int)
    parser.add_argument("--ttac_update_gate", type=str)
    parser.add_argument("--ttac_update_tv_threshold", type=float)
    parser.add_argument("--ttac_update_value_margin", type=float)
    parser.add_argument("--ttac_cache_estimator_target", type=int)
    parser.add_argument("--ttac_v5_8_logit_step_size", type=float)
    parser.add_argument("--ttac_v5_8_bias_clip", type=float)
    parser.add_argument("--ttac_v5_8_bias_decay", type=float)
    parser.add_argument("--ttac_v5_8_latent_dim", type=int)
    parser.add_argument("--ttac_v5_8_latent_scale", type=float)
    parser.add_argument("--ttac_policy_bank_history_len", type=int)
    parser.add_argument("--ttac_policy_bank_temp", type=float)
    parser.add_argument("--ttac_policy_bank_score_mode", type=str)
    parser.add_argument("--ttac_policy_bank_indices", type=str)
    parser.add_argument("--ttac_policy_bank_blend_alpha", type=float)
    parser.add_argument("--ttac_policy_bank_blend_clip", type=float)
    parser.add_argument("--ttac_latent_decoder_path", type=str)
    parser.add_argument("--ttac_latent_decoder_history_len", type=int)
    parser.add_argument("--ttac_latent_decoder_history_mode", type=str)
    parser.add_argument("--ttac_latent_decoder_blend_alpha", type=float)
    parser.add_argument("--ttac_latent_decoder_blend_clip", type=float)
    ...
    latent_decoder_params = (
        load_latent_decoder_npz(args.ttac_latent_decoder_path)
        if args.ttac_latent_decoder_path
        else None
    )
    ...
    model_overrides = {
        ...
        "TTAC_UPDATE_TV_THRESHOLD": args.ttac_update_tv_threshold,
        "TTAC_UPDATE_VALUE_MARGIN": args.ttac_update_value_margin,
        "TTAC_CACHE_ESTIMATOR_TARGET": None if args.ttac_cache_estimator_target is None else bool(args.ttac_cache_estimator_target),
        ...
        "TTAC_V5_8_LATENT_DIM": args.ttac_v5_8_latent_dim,
        "TTAC_V5_8_LATENT_SCALE": args.ttac_v5_8_latent_scale,
        ...
        "TTAC_LATENT_DECODER": latent_decoder_params,
