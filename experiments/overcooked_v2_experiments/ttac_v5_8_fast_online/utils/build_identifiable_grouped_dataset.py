...
def main():
    ...
    entries = load_policy_entries(args)
    ...
    if not np.any(available):
        raise ValueError("none of the requested policies has support rows in source_dataset")
    if not np.all(available):
        print(
            f"[identifiable_dataset] skip policies without support: {policy_ids[~available].tolist()}",
            flush=True,
        )
    ...
    for pos, entry in enumerate(entries):
        ...
        print(
            f"[identifiable_dataset] teacher {pos + 1}/{len(entries)} {policy_id}",
            flush=True,
        )
    ...
    query_budget = min(int(args.max_queries), int(args.max_rows) // max(1, int(args.policies_per_query)))
    ...
    arrays = {
        ...
        "pair_label": np.asarray(
            [
                f"identifiable:{policy_id}@{source_pair_labels[src]}"
    ...
    dataset_path = output / "identifiable_grouped_latent_decoder_dataset.npz"
    ...
    with (output / "dataset_summary.md").open("w", encoding="utf-8") as f:
        f.write("# Identifiable grouped latent-decoder dataset\n\n")
        f.write(f"- source_dataset: `{args.source_dataset}`\n")
        f.write(f"- output_dataset: `{dataset_path}`\n")
        f.write(f"- requested_policy_indices: PPO `{args.ppo_policy_indices}`, MAPPO `{args.mappo_policy_indices}`\n")
        ...
        f.write("- mean_teacher_probs: mean policy distribution across all loaded PG policies for the query.\n")
        f.write("- support: selected policy partner obs/action history, different episode when available.\n")
    print(
        f"[identifiable_dataset] wrote {dataset_path} rows={len(arrays['ego_action'])}",
        flush=True,
    )
