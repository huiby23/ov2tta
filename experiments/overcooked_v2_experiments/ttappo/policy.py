...
from overcooked_v2_experiments.ttappo.models.abstract import ActorCriticBase
from overcooked_v2_experiments.ttappo.models.model import (
    get_actor_critic,
    initialize_carry,
)
import jax
...
def summarize_ttt_source_stats(params, config, key):
    ...
    def _policy_step(params, hstate, obs, done, key):
        next_hstate, pi, _, aux = _apply_policy_network(network, params, hstate, (obs, done))
    ...
    for _ in range(num_episodes):
        ...
        hstate = initialize_carry(config, 1)
        done_all = False
        steps = 0
...
def policy_checkoints_to_functional_policy_pairing(
    ...
    return FunctionalPolicyPairing(tuple(policies), backend="ttappo", config=config)
...
def get_functional_rollout(policies: FunctionalPolicyPairing, env, key):
    ...
    def _update_adaptive_policy(spec, tx, state, partner_action, done):
        def _loss_fn(params):
            ...
            return total, (feature_z, feature_s)

        (_, (feature_z, feature_s)), grads = jax.value_and_grad(_loss_fn, has_aux=True)(
            state.adapt_params
        )
        updates, new_opt_state = tx.update(grads, state.opt_state, state.adapt_params)
        new_params = optax.apply_updates(state.adapt_params, updates)
...
class AdaptivePPOPolicy(AbstractPolicy):
    ...
    def update_after_step(self, hstate, partner_obs, partner_action, done):
        ...
        def _loss_fn(params):
            ...
            aux = (feature_z, feature_s)
            return total, aux

        (loss, (feature_z, feature_s)), grads = jax.value_and_grad(
        ...
        new_params = optax.apply_updates(hstate.adapt_params, updates)
        queue_z, queue_ptr, queue_count = _insert_queue(
            hstate.queue_z, hstate.queue_ptr, hstate.queue_count, feature_z
        )
        queue_s, _, _ = _insert_queue(
