...
from overcooked_v2_experiments.ttappo.policy import (
    PPOParams,
    policy_checkoints_to_functional_policy_pairing,
)
from overcooked_v2_experiments.ttappo.utils.store import load_all_checkpoints
...
def visualize_ppo_policy(
    ...
    if ttt_mode == "no_adapt":
        adaptive_index = None
        config["model"]["TTT_SSL_COEF"] = 0.0
        config["model"]["TTT_ALIGN_Z_COEF"] = 0.0
        config["model"]["TTT_ALIGN_S_COEF"] = 0.0
    elif ttt_mode == "ce_only":
        adaptive_index = 0
        config["model"]["TTT_ALIGN_Z_COEF"] = 0.0
        config["model"]["TTT_ALIGN_S_COEF"] = 0.0
    elif ttt_mode == "full":
        ...
        raise ValueError(f"Unknown ttt_mode: {ttt_mode}")
    ...
    if cross:
        ...
        for run_combination in run_combinations:
            ...
            policy_combination = policy_checkoints_to_functional_policy_pairing(
    ...
    policy_pairings, treedef = jax.tree_util.tree_flatten(
        all_params, is_leaf=lambda x: type(x) is FunctionalPolicyPairing
    )
    ...
    summery_file = run_base_dir / summary_name
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ttt_mode", type=str, default="full", choices=["full", "ce_only", "no_adapt"])
