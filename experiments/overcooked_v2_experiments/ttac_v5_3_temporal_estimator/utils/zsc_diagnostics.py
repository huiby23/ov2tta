...
from overcooked_v2_experiments.ttac_v5_3_temporal_estimator.policy import PPOPolicy as PPODiagnosticsPolicy
from overcooked_v2_experiments.ttac_v5_3_temporal_estimator.utils.store import (
    load_all_checkpoints as load_ppo_all_checkpoints,
)
from overcooked_v2_experiments.ttac_v5_3_temporal_estimator.utils.visualize_ppo import (
...
def ensure_cross_reward_summary(
    ...
    if backend == "ppo":
        ...
        visualize_ppo_run(
            ...
            ttac_mode=eval_mode,
            output_tag=eval_mode,
            model_overrides=ttac_model_overrides,
        )
    else:
...
def load_policies(run_dir, backend="ppo", eval_mode="memory_off", ttac_model_overrides=None):
    ...
    if backend == "ttac" and ttac_model_overrides:
        for override_key, value in ttac_model_overrides.items():
...
def summarize_method(
    ...
    summary_path = ensure_cross_reward_summary(
        ...
        ttac_model_overrides=ttac_model_overrides,
    )
