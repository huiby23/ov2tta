...
from overcooked_v2_experiments.ttac_v5_3_temporal_estimator.sync_population_run import single_run
...
def _write_audit(run_base_dir: Path, config, num_runs: int):
    audit = {
        "method": "TTACv2-sync-MEP-population-overcooked-v2",
        "source_reference": {
        ...
        "faithfulness": {
            "migrated_objective": "maximize H(mean_k pi_k(a|s)) for partner population policies",
    ...
    with open(run_base_dir / "mep_reproduction_audit.json", "w") as f:
...
def single_run_with_viz(config):
    ...
    mep_cfg = config.get("MEP", {})
    pop_size = mep_cfg.get("POPULATION_SIZE", 5)
    run_name = f"mep_K{pop_size}_{model_name}_ov2_{layout_name}_{avs_str}"
    if optional_prefix:
    ...
    with wandb.init(
        ...
        tags=["TTACv2", "sync-population", "MEP-style", model_name, "OvercookedV2"],
        config=config,
    ...
    print("TTACv2 sync population run_dir", run_base_dir)
...
def main(config):
    print(config)
    if config["TUNE"]:
        raise NotImplementedError("MEP tune is not implemented.")
    if "NUM_ITERATIONS" in config:
        from overcooked_v2_experiments.population_state_sample_run import (
            population_state_sample_run,
        )
        from overcooked_v2_experiments.ttac_v5_3_temporal_estimator.sync_population_train import make_train

        population_state_sample_run(
            config,
            label="TTACv2-sync-population",
            make_train_fn=make_train,
            method_config_key="MEP",
        )
        return
