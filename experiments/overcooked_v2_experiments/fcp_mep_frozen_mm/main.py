...
from overcooked_v2_experiments.fcp_mep_frozen_mm.run import single_run
...
def _write_audit(run_base_dir: Path, config, num_runs: int):
    audit = {
        "method": "FCP-MEP-frozen-partner-with-MM-overcooked-v2",
        "source_reference": {
            "repo": "https://github.com/huiby23/ZSC/tree/PBL-subnet",
            "source_files": [
                "pyhanabi/r2d2.py:get_entropy",
                "pyhanabi/selfplay.py:div_type/div_weight/no_sharing",
                "pyhanabi/net.py:play_styles action-stack",
            ],
        },
        "faithfulness": {
            "migrated_objective": "train learner with PPO against frozen empirical partners in MP episodes plus learner-learner MM regularization",
            "not_migrated_by_design": "pure MP-only FCP; this variant adds MM regularization by design",
            "population_is_real": True,
            "partner_population_frozen": True,
            "partner_population_updated": False,
            "num_independent_runs": num_runs,
        },
        "config": config,
    }
    with open(run_base_dir / "fcp_mep_frozen_mm_reproduction_audit.json", "w") as f:
...
def single_run_with_viz(config):
    ...
    fcp_cfg = config.get("FCP_MEP_FROZEN_MM", {})
    pop_size = fcp_cfg.get("POPULATION_SIZE", 10)
    run_name = f"fcp_mep_frozen_mm_K{pop_size}_{model_name}_ov2_{layout_name}_{avs_str}"
    if optional_prefix:
    ...
    with wandb.init(
        ...
        tags=["FCP-MEP-Frozen-MM", model_name, "OvercookedV2", "fcp", "mm-regularized"],
        config=config,
    ...
    print("FCP-MEP-Frozen-MM run_dir", run_base_dir)
...
def main(config):
    print(config)
    if config["TUNE"]:
        raise NotImplementedError("FCP-MEP-Frozen-MM tune is not implemented.")
    if "NUM_ITERATIONS" in config:
        ...
        population_state_sample_run(
            config,
            label="FCP_MEP_FROZEN_MM",
            make_train_fn=make_train,
            method_config_key="FCP_MEP_FROZEN_MM",
        )
        return
