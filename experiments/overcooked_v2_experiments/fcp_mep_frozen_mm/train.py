"""FCP with self-play regularization on the MEP CNN population backbone.

This variant keeps the standard FCP idea in MP episodes: the learner trains
against a frozen empirical partner population.  It adds MM episodes where both
sides are controlled by the learner and included in the learner PPO loss, so the
resulting learner is not only a best response to frozen partners but also sees
learner-learner coordination during training.
"""
...
class FCPMEPTransition(NamedTuple):
    done: jnp.ndarray
    action: jnp.ndarray
    value: jnp.ndarray
    reward: jnp.ndarray
...
def make_train(config, update_step_offset=None, update_step_num_overwrite=None):
    ...
    fcp_config = config.get("FCP_MEP_FROZEN_MM", {})

    if model_config["TYPE"] != "CNN":
        raise NotImplementedError("FCP-MEP-Frozen-MM v1 intentionally supports PPO CNN only.")

    population_size = int(fcp_config.get("POPULATION_SIZE", 10))
    # FCP + MM regularization: MP uses frozen population partners, while MM uses
    # the learner on both sides and updates both learner-controlled actors.
    group_mm = int(fcp_config.get("GROUP_MM", 1))
    group_mp = int(fcp_config.get("GROUP_MP", 1))
    if group_mm < 0 or group_mp < 0 or group_mm + group_mp <= 0:
        raise ValueError("FCP-MEP-Frozen-MM requires GROUP_MM/GROUP_MP to be non-negative with positive sum.")
    ...
    print("fcp_mep_frozen_mm mm actors", mm_mask_flat.sum(), "mp actors", mp_mask_flat.sum())
    print("fcp_mep_frozen_mm learner update actors", ego_actor_mask_flat.sum(), "frozen partner actors", partner_actor_mask_flat.sum())

    def _update_checkpoint(checkpoint_states, params, i):
        jax.debug.print("Saving FCP-MEP-Frozen-MM checkpoint {i}", i=i)
    ...
    def train(
        ...
        ego_params = network.init(ego_init_rng, init_hstate, init_x)

        partner_keys = jax.random.split(partner_init_rng, population_size)
        partner_params = jax.vmap(lambda k: network.init(k, init_hstate, init_x))(
            partner_keys
        )
        ...
        partner_state = TrainState.create(
            apply_fn=network.apply, params=partner_params, tx=tx
        )
        ...
        def _forward_actor(params, obs_batch, done_batch):
            if obs_batch.ndim == len(env.observation_space().shape) + 1:
                ...
                ac_done = done_batch
                squeeze_time = False
            _, pi, value = network.apply(params, None, (ac_obs, ac_done))
            if squeeze_time:
                return pi.logits[0], value[0]
            return pi.logits, value

        def _all_partner_forward(params, obs_batch, done_batch):
            return jax.vmap(lambda p: _forward_actor(p, obs_batch, done_batch))(params)
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_step_state, unused):
                ...
                ego_logits, ego_value = _forward_actor(
                    ego_state.params, obs_batch, last_done
                )
                ...
                info["fcp_mep_frozen_mm_pair_partner_id_mean"] = jnp.full(
                    (model_config["NUM_ACTORS"],), partner_id_mean
                )
                info["fcp_mep_frozen_mm_pair_is_mp"] = jnp.broadcast_to(
                    mp_mask_flat.astype(jnp.float32), (model_config["NUM_ACTORS"],)
                )
                info["fcp_mep_frozen_mm_learner_update_mask"] = jnp.broadcast_to(
                    ego_actor_mask_flat.astype(jnp.float32), (model_config["NUM_ACTORS"],)
                )
                info["fcp_mep_frozen_mm_frozen_partner_mask"] = jnp.broadcast_to(
            ...
            ego_last_logits, ego_last_val = _forward_actor(
                ego_state.params, last_obs_batch, last_done
            )
            del ego_last_logits
            ...
            def _loss_fn(ego_params, partner_params, traj_batch, gae, targets):
                ego_logits, ego_value = _forward_actor(
                    ego_params, traj_batch.obs, traj_batch.done
                )
                ego_pi = distrax.Categorical(logits=ego_logits)
                ego_log_prob = ego_pi.log_prob(traj_batch.action)

                partner_logits_all, partner_values_all = jax.vmap(
                    lambda p: _forward_actor(p, traj_batch.obs, traj_batch.done)
                )(partner_params)
                ...
                del current_partner_mask
                all_partner_probs = jax.nn.softmax(jax.lax.stop_gradient(partner_logits_all), axis=-1)
                ...
                partner_total = jax.lax.stop_gradient(partner_ppo)
                total = ego_total
                ...
                aux = {
                    ...
                    "ego_ratio": ego_ratio,
                    "frozen_partner_diagnostic_loss": partner_total,
                    ...
                    "fcp_mep_frozen_mm_partner_population_entropy": mep_entropy,
                }
                return total, aux
            ...
            def _update_epoch(update_state, unused):
                ...
                def _update_minibatch(states, batch_info):
                    ...
                    (total_loss, aux), ego_grads = grad_fn(
                        ego_state.params,
                    ...
                    # MP partners are the frozen empirical FCP population; do not update them.
            ...
            metric["fcp_mep_frozen_mm_population_size"] = jnp.array(population_size, dtype=jnp.float32)
            metric["fcp_mep_frozen_mm_group_mm"] = jnp.array(group_mm, dtype=jnp.float32)
            metric["fcp_mep_frozen_mm_group_mp"] = jnp.array(group_mp, dtype=jnp.float32)
            update_step += 1
