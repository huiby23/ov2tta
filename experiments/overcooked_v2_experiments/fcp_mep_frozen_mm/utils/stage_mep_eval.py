...
def stage(run_dir: Path, run_num: int, mode: str, output: Path | None):
    ...
    if not src_run.is_dir():
        raise FileNotFoundError(f"missing MEP run directory: {src_run}")
    if output is None:
...
def main():
    parser = argparse.ArgumentParser(description="Stage MEP ego/partner checkpoints for PPO visualizer cross-play evaluation.")
