...
from overcooked_v2_experiments.ttac_v5_6_light_online.models.abstract import ActorCriticBase
from overcooked_v2_experiments.ttac_v5_6_light_online.models.model import get_actor_critic, initialize_carry
from overcooked_v2_experiments.ttac_v5_6_light_online.utils.surrogate_heads import (
    apply_q_eta,
    expected_joint_q,
)
from overcooked_v2_experiments.ttac_v5_6_light_online.utils.agreement_heads import (
...
class PPOPolicy(AbstractPolicy):
    ...
    def _ttac_latest_light_loss(
        ...
        if agreement_estimator_params is None:
            ...
            if cached_estimator_probs is None:
                estimator_probs = self._ttac_latest_light_estimator_probs(cached_context)
            else:
    ...
    def update_after_step(self, hstate, partner_obs, partner_action, done):
        ...
        if self.eval_mode == "ttac_v5_6_latest_light" and cache_estimator_target:
            cached_context = self._ttac_latest_light_context(hstate)
            cached_estimator_probs = self._ttac_latest_light_estimator_probs(
                cached_context
            )
        else:
            cached_context = None
...
def policy_checkoints_to_functional_policy_pairing(
    ...
    return FunctionalPolicyPairing(
        tuple(policies),
        backend="ttac_v5_6_light_online",
        config=config,
    )
