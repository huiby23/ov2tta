...
from overcooked_v2_experiments.mep.run import single_run
...
def _write_audit(run_base_dir: Path, config, num_runs: int):
    audit = {
        "method": "MEP-real-population-overcooked-v2",
        "source_reference": {
...
def single_run_with_viz(config):
    ...
    with wandb.init(
        ...
        tags=["MEP", model_name, "OvercookedV2", "real-population"],
        config=config,
    ...
    print("MEP run_dir", run_base_dir)
...
def main(config):
    ...
    if "NUM_ITERATIONS" in config:
        ...
        from overcooked_v2_experiments.mep.train import make_train
