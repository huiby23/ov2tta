...
from overcooked_v2_experiments.ttac_v5_8_fast_online.policy import PPOParams, PPOPolicy  # noqa: E402
...
def infer_latent_decoder_z_dim(decoder_bundle) -> int | None:
    ...
    if decoder is None:
        return None
    if "latent_online_residual_c_enabled" in decoder and "vae_mu_b" in decoder and "orc_c_init" in decoder:
        return int(np.asarray(decoder["vae_mu_b"]).shape[0] + np.asarray(decoder["orc_c_init"]).shape[0])
...
def _param_tree_has_path(tree, target_path: str) -> bool:
    found = False
...
def infer_legacy_cnn_params(params) -> bool:
    has_legacy_actor = _param_tree_has_path(params, "params/Dense_0/kernel")
    has_v58_actor = _param_tree_has_path(params, "params/base_actor_hidden/kernel")
    return bool(has_legacy_actor and not has_v58_actor)
...
METHOD_SPECS = {
    ...
    "pqn_soft": {
        "subdir": "pqn_soft",
        "file_prefix": "pqn_soft_cnn",
        "module": pqn_soft_cnn_overcooked,
    },
    "pqn_wqmix": {
        "subdir": "pqn_wqmix",
        "file_prefix": "pqn_wqmix_cnn",
        "module": pqn_wqmix_cnn_overcooked,
    },
    "pqn_qplex": {
        "subdir": "pqn_qplex",
        "file_prefix": "pqn_qplex_cnn",
...
def load_q_method(run_root: Path, method: str, layout: str, max_policies: int | None) -> LoadedQMethod:
    ...
    elif method in {"pqn_vdn", "pqn_soft", "pqn_wqmix", "pqn_qplex"}:
        network = spec["module"].QNetwork(
            action_dim=action_dim,
...
def load_ego_policies(
    ...
    all_params, config = load_ego_checkpoints(
        run_dir,
        final_only=True,
    ...
    first_run_key = sorted(all_params.keys(), key=lambda x: int(x.split("_")[1]))[0]
    first_ckpt = all_params[first_run_key].get("ckpt_final")
    if first_ckpt is not None and infer_legacy_cnn_params(first_ckpt.params):
        config["model"]["TTAC_LEGACY_CNN"] = True
...
def load_ego_checkpoints(
    run_dir: Path,
    final_only: bool = True,
    checkpoint_subdir: str = "",
) -> tuple[dict, dict]:
    """Load ego checkpoints, optionally from run_X/<checkpoint_subdir>/ckpt_final."""
    checkpoint_subdir = checkpoint_subdir.strip("/")
    if not checkpoint_subdir:
        return load_all_checkpoints(run_dir, final_only=final_only)
    ...
    for run_num_dir in run_dirs:
        checkpoint_root = run_num_dir / checkpoint_subdir
        ...
        checkpoint_dirs = sorted(
            ...
            key=lambda path: (path.name != "ckpt_final", _natural_vmap_key(path)),
        )
        for checkpoint_dir in checkpoint_dirs:
            if final_only and "final" not in checkpoint_dir.name:
                continue
            ckpt = checkpointer.restore(checkpoint_dir.resolve(), item=None)
            checkpoints[checkpoint_dir.name] = PPOParams(params=ckpt["params"])
            if first_config is None:
                first_config = ckpt["config"]
    ...
    if not all_checkpoints:
        raise FileNotFoundError(
            f"no ego checkpoints loaded from {run_dir} with subdir {checkpoint_subdir!r}"
        )
...
def write_markdown(path: Path, summary_rows: Sequence[dict], args: argparse.Namespace):
    lines = [
        ...
        f"- Ego checkpoint subdir: `{args.ego_checkpoint_subdir}`",
        ...
        f"- Latent decoder online smooth coef: `{args.ttac_latent_decoder_online_smooth_coef}`",
        f"- Latent decoder online min history: `{args.ttac_latent_decoder_online_min_history}`",
        f"- Latent decoder online support fraction: `{args.ttac_latent_decoder_online_support_fraction}`",
        f"- Latent decoder online init z mode: `{args.ttac_latent_decoder_online_init_z_mode}`",
        f"- Latent decoder online LoRA lr: `{args.ttac_latent_decoder_online_lora_lr}`",
        f"- Latent decoder online LoRA update steps: `{args.ttac_latent_decoder_online_lora_update_steps}`",
        f"- Latent decoder online LoRA anchor KL coef: `{args.ttac_latent_decoder_online_lora_anchor_kl_coef}`",
        f"- Latent decoder online LoRA L2 coef: `{args.ttac_latent_decoder_online_lora_l2_coef}`",
        f"- Latent decoder online LoRA val steps: `{args.ttac_latent_decoder_online_lora_val_steps}`",
        f"- Latent decoder online LoRA accept validation: `{args.ttac_latent_decoder_online_lora_accept_validation}`",
        f"- Latent decoder online LoRA accept margin: `{args.ttac_latent_decoder_online_lora_accept_margin}`",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ego_checkpoint_subdir",
        type=str,
        default="",
        help="Optional nested checkpoint subdirectory under each run_X, e.g. 'ego' for run_X/ego/ckpt_final.",
    )
    parser.add_argument(
        "--q_run_root",
        type=Path,
    ...
    parser.add_argument("--ttac_latent_decoder_online_smooth_coef", type=float, default=0.0)
    parser.add_argument("--ttac_latent_decoder_online_min_history", type=int, default=10)
    parser.add_argument("--ttac_latent_decoder_online_support_fraction", type=float, default=1.0)
    ...
    parser.add_argument("--ttac_latent_decoder_online_lora_lr", type=float, default=1e-3)
    parser.add_argument("--ttac_latent_decoder_online_lora_update_steps", type=int, default=5)
    parser.add_argument("--ttac_latent_decoder_online_lora_anchor_kl_coef", type=float, default=0.05)
    parser.add_argument("--ttac_latent_decoder_online_lora_l2_coef", type=float, default=1e-3)
    parser.add_argument("--ttac_latent_decoder_online_lora_val_steps", type=int, default=10)
    parser.add_argument("--ttac_latent_decoder_online_lora_accept_margin", type=float, default=0.0)
    parser.add_argument(
        "--ttac_latent_decoder_online_lora_accept_validation",
...
def main():
    ...
    model_overrides = {
        ...
        "TTAC_LATENT_DECODER_ONLINE_SMOOTH_COEF": args.ttac_latent_decoder_online_smooth_coef,
        "TTAC_LATENT_DECODER_ONLINE_MIN_HISTORY": args.ttac_latent_decoder_online_min_history,
        "TTAC_LATENT_DECODER_ONLINE_SUPPORT_FRACTION": args.ttac_latent_decoder_online_support_fraction,
        ...
        "TTAC_LATENT_DECODER_ONLINE_LORA_LR": args.ttac_latent_decoder_online_lora_lr,
        "TTAC_LATENT_DECODER_ONLINE_LORA_UPDATE_STEPS": args.ttac_latent_decoder_online_lora_update_steps,
        "TTAC_LATENT_DECODER_ONLINE_LORA_ANCHOR_KL_COEF": args.ttac_latent_decoder_online_lora_anchor_kl_coef,
        "TTAC_LATENT_DECODER_ONLINE_LORA_L2_COEF": args.ttac_latent_decoder_online_lora_l2_coef,
        "TTAC_LATENT_DECODER_ONLINE_LORA_VAL_STEPS": args.ttac_latent_decoder_online_lora_val_steps,
        "TTAC_LATENT_DECODER_ONLINE_LORA_ACCEPT_VALIDATION": args.ttac_latent_decoder_online_lora_accept_validation,
        "TTAC_LATENT_DECODER_ONLINE_LORA_ACCEPT_MARGIN": args.ttac_latent_decoder_online_lora_accept_margin,
    ...
    for ego_mode in ego_modes:
        ...
        ego_policies, ego_config = load_ego_policies(
            ...
            checkpoint_subdir=args.ego_checkpoint_subdir,
        )
