"""FCP-style co-play training on the MEP real-population CNN backbone.

This variant intentionally keeps the MEP training mechanism: one ego policy and
a real population of independent partner policies are rolled out together and
updated together with PPO under the same MM/MP pairing masks.  The only removed
component is MEP's diversity entropy objective, making this the no-diversity
FCP/co-play control for the MEP architecture.
"""
...
def make_train(config, update_step_offset=None, update_step_num_overwrite=None):
    ...
    fcp_config = config.get("FCP_MEP", {})

    if model_config["TYPE"] != "CNN":
        raise NotImplementedError("FCP-MEP v1 intentionally supports PPO CNN only.")

    population_size = int(fcp_config.get("POPULATION_SIZE", 5))
    # Match MEP's training mechanism by default: both MM and MP pairings, with
    # ego and partner-population policies updated from the same rollout.
    ...
    if group_mm < 0 or group_mp < 0 or group_mm + group_mp <= 0:
        raise ValueError("FCP-MEP requires GROUP_MM/GROUP_MP to be non-negative with positive sum.")
    ...
    print("fcp_mep mm actors", mm_mask_flat.sum(), "mp actors", mp_mask_flat.sum())
    print("fcp_mep ego update actors", ego_actor_mask_flat.sum(), "partner update actors", partner_actor_mask_flat.sum())

    def _update_checkpoint(checkpoint_states, params, i):
        jax.debug.print("Saving FCP-MEP checkpoint {i}", i=i)
    ...
    def train(
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_step_state, unused):
                ...
                info["fcp_mep_pair_partner_id_mean"] = jnp.full(
                    (model_config["NUM_ACTORS"],), partner_id_mean
                )
                info["fcp_mep_pair_is_mp"] = jnp.broadcast_to(
                    mp_mask_flat.astype(jnp.float32), (model_config["NUM_ACTORS"],)
                )
                info["fcp_mep_ego_update_mask"] = jnp.broadcast_to(
                    ego_actor_mask_flat.astype(jnp.float32), (model_config["NUM_ACTORS"],)
                )
                info["fcp_mep_partner_update_mask"] = jnp.broadcast_to(
            ...
            def _loss_fn(ego_params, partner_params, traj_batch, gae, targets):
                ...
                aux = {
                    ...
                    "fcp_mep_partner_population_entropy": mep_entropy,
                }
                return total, aux
            ...
            metric["fcp_mep_population_size"] = jnp.array(population_size, dtype=jnp.float32)
            metric["fcp_mep_group_mm"] = jnp.array(group_mm, dtype=jnp.float32)
            metric["fcp_mep_group_mp"] = jnp.array(group_mp, dtype=jnp.float32)
            update_step += 1
