...
from overcooked_v2_experiments.fcp_mep.run import single_run
...
def _write_audit(run_base_dir: Path, config, num_runs: int):
    audit = {
        "method": "FCP-MEP-coplay-population-overcooked-v2",
        "source_reference": {
        ...
        "faithfulness": {
            "migrated_objective": "train ego and real partner-population policies together with PPO under MEP MM/MP pairing masks",
            "not_migrated_by_design": "MEP diversity entropy objective; removed to form the FCP/co-play control",
            "population_is_real": True,
            "partner_population_frozen": False,
            "partner_population_updated": True,
            "num_independent_runs": num_runs,
        },
        "config": config,
    }
    with open(run_base_dir / "fcp_mep_reproduction_audit.json", "w") as f:
...
def single_run_with_viz(config):
    ...
    fcp_cfg = config.get("FCP_MEP", {})
    pop_size = fcp_cfg.get("POPULATION_SIZE", 5)
    run_name = f"fcp_mep_K{pop_size}_{model_name}_ov2_{layout_name}_{avs_str}"
    if optional_prefix:
    ...
    with wandb.init(
        ...
        tags=["FCP-MEP", model_name, "OvercookedV2", "coplay-population"],
        config=config,
    ...
    print("FCP-MEP run_dir", run_base_dir)
...
def main(config):
    print(config)
    if config["TUNE"]:
        raise NotImplementedError("FCP-MEP tune is not implemented.")
    if "NUM_ITERATIONS" in config:
        from overcooked_v2_experiments.population_state_sample_run import (
            population_state_sample_run,
        )
        from overcooked_v2_experiments.fcp_mep.train import make_train

        population_state_sample_run(
            config,
            label="FCP_MEP",
            make_train_fn=make_train,
            method_config_key="FCP_MEP",
        )
        return
