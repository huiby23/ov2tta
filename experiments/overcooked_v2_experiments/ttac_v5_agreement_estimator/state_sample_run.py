...
from overcooked_v2_experiments.ttac_v5_agreement_estimator.models.model import (
    get_actor_critic,
    initialize_carry,
)
from overcooked_v2_experiments.ttac_v5_agreement_estimator.policy import (
    PPOParams,
    PPOPolicy,
    policy_checkoints_to_policy_pairing,
)
from overcooked_v2_experiments.ttac_v5_agreement_estimator.utils.store import store_checkpoint
from overcooked_v2_experiments.ttac_v5_agreement_estimator.utils.utils import (
    combine_first_two_tree_dim,
    get_num_devices,
    get_run_base_dir,
)
from overcooked_v2_experiments.ttac_v5_agreement_estimator.utils.visualize_ppo import visualize_ppo_policy
...
def state_sample_run(config):
    ...
    frozen_population_policy = None
    if "TTAC_V2_FROZEN_POPULATION" in config:
        if hp_policy is not None:
            raise ValueError("BC and TTAC_V2_FROZEN_POPULATION cannot both be used")
        from overcooked_v2_experiments.ttac_v5_agreement_estimator.population_policy import (
            FrozenCheckpointPopulationPolicy,
        )

        pop_cfg = config["TTAC_V2_FROZEN_POPULATION"]
        print(
            "Training state-aug with frozen checkpoint population",
            pop_cfg,
            flush=True,
        )
        frozen_population_policy = FrozenCheckpointPopulationPolicy.from_run_dir(
            pop_cfg["RUN_DIR"],
            max_policies=pop_cfg.get("MAX_POLICIES", 10),
            stochastic=pop_cfg.get("STOCHASTIC", True),
        )
    ...
    with wandb.init(
        ...
        def _run_iteration(
            ...
            if frozen_population_policy is not None:
                base_train_fn = train_fn

                def train_fn(rng, run_index=None, initial_train_state=None):
                    return base_train_fn(
                        rng,
                        run_index=run_index,
                        population=frozen_population_policy,
                        initial_train_state=initial_train_state,
                    )

                print(
                    "Training state-aug iteration with frozen checkpoint population",
                    flush=True,
                )
        ...
        def _collect_states(previous_policies, key):
            ...
            def _process_combination_wrapper(
                ...
                def _process_combination(policies, key):
                    ...
                    sampled_state_sequences = jax.tree_map(
            ...
            num_actors = 2
            if frozen_population_policy is not None:
                state_collect_func = _process_combination_wrapper(
                    num_rollouts=10, state_step_size=10
                )

                key, subkey = jax.random.split(key)
                collect_keys = jax.random.split(subkey, num_runs * num_actors)
                state_batches = []
                collect_idx = 0
                for run_num in range(num_runs):
                    ...
                    ppo_pop_pairing = PolicyPairing(
                        ppo_policy, frozen_population_policy
                    )
                    pop_ppo_pairing = PolicyPairing(
                        frozen_population_policy, ppo_policy
                    )
                    state_batches.append(
                        state_collect_func(
                            ppo_pop_pairing, collect_keys[collect_idx]
                        )
                    )
                    collect_idx += 1
                    state_batches.append(
                        state_collect_func(
                            pop_ppo_pairing, collect_keys[collect_idx]
                        )
                    )
                    collect_idx += 1

                state_buffer = jax.tree_util.tree_map(
                    lambda *xs: jnp.concatenate(xs, axis=0), *state_batches
                )
            elif hp_policy is None:
            ...
            print(
                "State buffer shape: ",
                jax.tree_map(lambda x: x.shape, state_buffer),
            )
