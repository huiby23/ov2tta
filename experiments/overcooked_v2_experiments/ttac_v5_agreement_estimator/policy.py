...
from overcooked_v2_experiments.ttac_v5_agreement_estimator.models.abstract import ActorCriticBase
from overcooked_v2_experiments.ttac_v5_agreement_estimator.models.model import get_actor_critic, initialize_carry
from overcooked_v2_experiments.ttac_v5_agreement_estimator.utils.surrogate_heads import (
    apply_q_eta,
    expected_joint_q,
)
from overcooked_v2_experiments.ttac_v5_agreement_estimator.utils.agreement_heads import (
...
class PPOPolicy(AbstractPolicy):
    ...
    def _ttac_loss(self, params, hstate, use_kl):
        ...
        if agreement_estimator_params is None:
            ...
            query_obs = hstate.ego_obs_buffer[latest_idx][None, ...]
            latest_partner_obs = hstate.partner_obs_buffer[latest_idx][None, ...]
            latest_ego_logits = ego_logits[latest_idx][None, :]
            ...
            ego_latest_log_probs = jax.nn.log_softmax(latest_ego_logits, axis=-1)
            ego_latest_probs = jax.nn.softmax(latest_ego_logits, axis=-1)
            v5_agreement_loss = -jnp.sum(estimator_probs * ego_latest_log_probs, axis=-1).mean()
            estimator_log_probs = jnp.log(jnp.maximum(estimator_probs, 1e-6))
            v5_estimator_kl = jnp.sum(
                estimator_probs * (estimator_log_probs - ego_latest_log_probs), axis=-1
            ).mean()
            v5_estimator_tv = 0.5 * jnp.sum(
                jnp.abs(estimator_probs - ego_latest_probs), axis=-1
            ).mean()
            v5_estimator_entropy = (
                -jnp.sum(estimator_probs * estimator_log_probs, axis=-1)
            ).mean()
