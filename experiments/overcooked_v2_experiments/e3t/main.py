...
from overcooked_v2_experiments.e3t.utils.utils import get_run_base_dir
...
def _store_all_checkpoints(config, out, num_checkpoints):
    from overcooked_v2_experiments.e3t.utils.store import store_checkpoint
...
def single_run_with_checkpoint(config):
    from overcooked_v2_experiments.e3t.run import single_run

    config = OmegaConf.to_container(config)
    if config["model"]["TYPE"] != "RNN":
        raise NotImplementedError("E3T currently supports the RNN configuration only.")
    if config.get("TUNE", False):
        raise NotImplementedError("E3T tuning entry is not implemented yet.")
    if "NUM_ITERATIONS" in config:
        raise NotImplementedError("E3T state augmentation entry is not implemented yet.")
    ...
    run_name = f"e3t_{model_name.lower()}_ov2_{layout_name}_{avs_str}"
    if optional_prefix:
    ...
    run = wandb.init(
        ...
        tags=["E3T", model_name, "OvercookedV2"],
        config=config,
