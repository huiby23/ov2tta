...
class ActorCriticCNN(ActorCriticBase):
    def setup(self):
        ...
        self.partner_gate_layer = nn.Dense(
            self.config["FC_DIM_SIZE"],
            kernel_init=orthogonal(0.01),
            bias_init=constant(0.0),
            name="partner_gate",
    ...
    def _ttt_forward_from_base(self, cached_feature):
        ...
        partner_gate = None
        actor_feature = feature_z
        if self.config.get("ACTOR_TEMPORAL_MODE", "base") == "gate":
            gate_input = temporal_feature
            if self.config.get("ACTOR_GATE_STOP_GRAD", True):
                gate_input = jax.lax.stop_gradient(gate_input)
            partner_gate = nn.sigmoid(self.partner_gate_layer(gate_input))
            gate_scale = self.config.get("ACTOR_GATE_SCALE", 0.2)
            actor_feature = feature_z * (1.0 + gate_scale * (partner_gate - 0.5))

        return actor_feature, feature_z, partner_hidden, partner_logits, partner_gate
    ...
    def __call__(self, hidden, x):
        ...
        actor_feature, feature_z, feature_s, partner_logits, partner_gate = self._ttt_forward_from_base(
            cached_feature
        )
