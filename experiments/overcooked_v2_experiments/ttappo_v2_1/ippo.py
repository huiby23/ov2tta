...
from overcooked_v2_experiments.ttappo_v2_1.models.abstract import ActorCriticBase
...
def get_partner_aux_coef(model_config, update_step):
    total_updates = float(model_config["NUM_UPDATES"])
    start_frac = float(model_config.get("PARTNER_AUX_START_FRAC", 0.0))
    ramp_frac = float(model_config.get("PARTNER_AUX_RAMP_FRAC", 0.0))
    target_coef = float(
        model_config.get(
            "PARTNER_AUX_TARGET_COEF", model_config.get("TTT_PARTNER_LOSS_COEF", 0.0)
        )
    )

    progress = jnp.asarray(update_step, dtype=jnp.float32) / jnp.maximum(
        total_updates, 1.0
    )
    ramp_end = start_frac + ramp_frac
    warm_ratio = (progress - start_frac) / max(ramp_frac, 1e-6)
    warm_ratio = jnp.clip(warm_ratio, 0.0, 1.0)
    coef = jnp.where(progress < start_frac, 0.0, target_coef * warm_ratio)
    coef = jnp.where(progress >= ramp_end, target_coef, coef)
...
def make_train(
    ...
    def train(
        ...
        def _update_step(runner_state, unused):
            ...
            partner_aux_coef = get_partner_aux_coef(model_config, update_step)
            ...
            def _env_step(env_step_state, unused):
                ...
                hstate, pi, value, _ = _apply_training_network(
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(
                        params, init_hstate, traj_batch, gae, targets, partner_aux_coef
                    ):
                        hstate = init_hstate
                        ...
                        partner_loss = optax.softmax_cross_entropy_with_integer_labels(
                            partner_logits, traj_batch.partner_action
                        ).mean(where=train_mask)
                        partner_pred = jnp.argmax(partner_logits, axis=-1)
                        partner_acc = (
                            (partner_pred == traj_batch.partner_action)
                            .astype(jnp.float32)
                            .mean(where=train_mask)
                        )
                        if "partner_gate" in aux:
                            partner_gate_mean = aux["partner_gate"].mean()
                        else:
                            partner_gate_mean = jnp.array(0.0, dtype=jnp.float32)
            ...
            value_loss, loss_actor, entropy, ratio, partner_loss, partner_acc, partner_gate_mean = aux_data
            ...
            metric["partner_aux_coef"] = partner_aux_coef
            metric["partner_gate_mean"] = partner_gate_mean
