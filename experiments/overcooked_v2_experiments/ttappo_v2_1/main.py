...
from overcooked_v2_experiments.ttappo_v2_1.policy import PPOParams, summarize_ttt_source_stats
from overcooked_v2_experiments.ttappo_v2_1.utils.store import load_all_checkpoints, store_checkpoint
from overcooked_v2_experiments.ttappo_v2_1.utils.utils import get_run_base_dir
...
def single_run_with_viz(config):
    from overcooked_v2_experiments.ttappo_v2_1.run import single_run
    ...
    if config["NUM_CHECKPOINTS"] > 0:
        ...
        for run_num in range(num_runs):
            for checkpoint in range(num_checkpoints):
                ...
                ttt_source_stats = {}
                if checkpoint == num_checkpoints - 1 and config["model"].get(
                    "COMPUTE_TTT_SOURCE_STATS", False
                ):
...
def main(config):
    ...
    if config["TUNE"]:
        from overcooked_v2_experiments.ttappo_v2_1.tune import tune

        tune(config)
    elif "NUM_ITERATIONS" in config:
        from overcooked_v2_experiments.ttappo_v2_1.state_sample_run import state_sample_run
