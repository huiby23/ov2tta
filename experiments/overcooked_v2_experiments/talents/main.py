...
from overcooked_v2_experiments.talents.run import save_ppo_style_checkpoints, single_run
from overcooked_v2_experiments.talents.utils.utils import get_run_base_dir
from overcooked_v2_experiments.talents.utils.visualize_ppo import visualize_ppo_policy
...
def main(config):
    ...
    if config.get("TUNE", False):
        raise NotImplementedError("TALENTS tuning entrypoint is not implemented yet.")
    ...
    run_name = f"talents_{model_name}_ov2_{layout_name}_{avs_str}"
    if optional_prefix:
        run_name = f"{optional_prefix}_{run_name}"
    with wandb.init(
        entity=config["wandb"]["ENTITY"],
        project=config["wandb"]["PROJECT"],
        tags=["TALENTS", model_name, "OvercookedV2", "strategy-conditioned"],
        config=config,
    ...
    print("TALENTS run_dir", run_base_dir)
