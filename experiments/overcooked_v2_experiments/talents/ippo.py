...
def make_train(
    ...
    use_priority_sampling = bool(talents_config.get("PRIORITY_SAMPLING", False))
    priority_mode = str(talents_config.get("PRIORITY_MODE", "low_return"))
    priority_ema_alpha = float(talents_config.get("PRIORITY_EMA_ALPHA", 0.05))
    priority_temperature = float(talents_config.get("PRIORITY_TEMPERATURE", 40.0))
    priority_uniform_mix = float(talents_config.get("PRIORITY_UNIFORM_MIX", 0.1))
    if use_priority_sampling:
        print(
            "Using TALENTS priority sampling",
            priority_mode,
            "ema",
            priority_ema_alpha,
            "temp",
    ...
    def _cluster_priority_probs(cluster_return_ema, cluster_return_counts):
        k = cluster_return_ema.shape[0]
        seen = cluster_return_counts > 0
        seen_count = jnp.maximum(seen.sum(), 1)
        seen_mean = jnp.sum(jnp.where(seen, cluster_return_ema, 0.0)) / seen_count
        scores = jnp.where(seen, cluster_return_ema, seen_mean)
        logits = scores / max(priority_temperature, 1e-6)
        if priority_mode in ("low_return", "hard", "paper"):
            logits = -logits
        probs = jax.nn.softmax(logits)
        probs = (1.0 - priority_uniform_mix) * probs + priority_uniform_mix / k
        return probs / jnp.maximum(probs.sum(), 1e-8)
    ...
    def train(
        ...
        init_x = (
            ...
            jnp.zeros((1, model_config["NUM_ENVS"]), dtype=jnp.int32),
        )
        ...
        num_strategy_clusters = int(config.get("TALENTS", {}).get("NUM_CLUSTERS", model_config.get("NUM_STRATEGY_CLUSTERS", 1)))
        model_config["NUM_STRATEGY_CLUSTERS"] = num_strategy_clusters
        rng, _rng = jax.random.split(rng)
        env_strategy_cluster_ids = jax.random.randint(
            _rng, (model_config["NUM_ENVS"],), 0, num_strategy_clusters, dtype=jnp.int32
        )
        cluster_return_ema = jnp.zeros((num_strategy_clusters,), dtype=jnp.float32)
        cluster_return_counts = jnp.zeros((num_strategy_clusters,), dtype=jnp.float32)
        env_episode_returns = jnp.zeros((model_config["NUM_ENVS"],), dtype=jnp.float32)
        ...
        def _update_step(runner_state, unused):
            (
                ...
                env_strategy_cluster_ids,
                cluster_return_ema,
                cluster_return_counts,
                env_episode_returns,
                rng,
            ) = runner_state
            ...
            def _env_step(env_step_state, unused):
                ...
                strategy_cluster_ids = jnp.tile(env_strategy_cluster_ids, env.num_agents)
                ac_in = (
                    obs_batch[np.newaxis, :],
                    last_done[np.newaxis, :],
                    strategy_cluster_ids[np.newaxis, :],
                )
                ...
                if population is not None:
                    ...
                    if is_policy_population:
                        rng, _rng = jax.random.split(rng)
                        if hasattr(population, "set_cluster_ids"):
                            population_hstate = population.set_cluster_ids(
                                population_hstate, strategy_cluster_ids, _rng
                            )
                            rng, _rng = jax.random.split(rng)
                ...
                step_team_reward = reward["agent_0"]
                ...
                completed_env_returns = env_episode_returns + step_team_reward
                done_env = done["__all__"].astype(jnp.float32)
                cluster_return_sums = jnp.bincount(
                    env_strategy_cluster_ids,
                    weights=completed_env_returns * done_env,
                    length=num_strategy_clusters,
                )
                cluster_done_counts = jnp.bincount(
                    env_strategy_cluster_ids,
                    weights=done_env,
                    length=num_strategy_clusters,
                )
                cluster_batch_mean = cluster_return_sums / jnp.maximum(cluster_done_counts, 1.0)
                cluster_has_update = cluster_done_counts > 0
                cluster_has_seen = cluster_return_counts > 0
                cluster_ema_candidate = jnp.where(
                    cluster_has_seen,
                    (1.0 - priority_ema_alpha) * cluster_return_ema
                    + priority_ema_alpha * cluster_batch_mean,
                    cluster_batch_mean,
                )
                new_cluster_return_ema = jnp.where(
                    use_priority_sampling & cluster_has_update,
                    cluster_ema_candidate,
                    cluster_return_ema,
                )
                new_cluster_return_counts = jnp.where(
                    use_priority_sampling,
                    cluster_return_counts + cluster_done_counts,
                    cluster_return_counts,
                )
                new_env_episode_returns = jnp.where(
                    done["__all__"],
                    jnp.zeros_like(completed_env_returns),
                    completed_env_returns,
                )

                rng, _rng = jax.random.split(rng)
                priority_probs = _cluster_priority_probs(
                    new_cluster_return_ema, new_cluster_return_counts
                )
                sampled_priority_clusters = jax.random.categorical(
                    _rng,
                    jnp.log(priority_probs + 1e-8),
                    shape=(model_config["NUM_ENVS"],),
                ).astype(jnp.int32)
                sampled_uniform_clusters = jax.random.randint(
                    _rng,
                    (model_config["NUM_ENVS"],),
                    0,
                    num_strategy_clusters,
                    dtype=jnp.int32,
                )
                sampled_clusters = jnp.where(
                    use_priority_sampling,
                    sampled_priority_clusters,
                    sampled_uniform_clusters,
                )
                new_env_strategy_cluster_ids = jnp.where(
                    done["__all__"],
                    sampled_clusters,
                    env_strategy_cluster_ids,
                )
                ...
                env_step_state = (
                    ...
                    new_env_strategy_cluster_ids,
                    new_cluster_return_ema,
                    new_cluster_return_counts,
                    new_env_episode_returns,
                    rng,
                )
            ...
            (
                ...
                next_env_strategy_cluster_ids,
                next_cluster_return_ema,
                next_cluster_return_counts,
                next_env_episode_returns,
                rng,
            ) = env_step_state
            ...
            last_strategy_cluster_ids = jnp.tile(next_env_strategy_cluster_ids, env.num_agents)
            ac_in = (
                last_obs_batch[np.newaxis, :],
                last_done[np.newaxis, :],
                last_strategy_cluster_ids[np.newaxis, :],
            )
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        _, pi, value = network.apply(
                            params,
                            hstate,
                            (traj_batch.obs, traj_batch.done, traj_batch.cluster_id),
                        )
            ...
            priority_probs = _cluster_priority_probs(
                next_cluster_return_ema, next_cluster_return_counts
            )
            metric["talents_priority_return_mean"] = next_cluster_return_ema.mean()
            metric["talents_priority_return_max"] = next_cluster_return_ema.max()
            metric["talents_priority_return_min"] = next_cluster_return_ema.min()
            metric["talents_priority_prob_max"] = priority_probs.max()
            metric["talents_priority_entropy"] = -jnp.sum(
                priority_probs * jnp.log(priority_probs + 1e-8)
            )
