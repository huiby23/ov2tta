"""TrajeDi-style sim-MI population training for OvercookedV2.

This module ports the PBL-subnet div_type=0 objective to OvercookedV2 with a
real partner population: each partner is an independent PPO parameter tree, not
a subnet/play-style head.
"""
...
class TrajeDiTransition(NamedTuple):
    done: jnp.ndarray
    action: jnp.ndarray
    value: jnp.ndarray
    reward: jnp.ndarray
...
def make_train(config, update_step_offset=None, update_step_num_overwrite=None):
    ...
    trajedi_config = config.get("TRAJEDI", {})

    if model_config["TYPE"] != "CNN":
        raise NotImplementedError("TrajeDi v1 intentionally supports PPO CNN only.")

    population_size = int(trajedi_config.get("POPULATION_SIZE", 5))
    trajedi_div_weight = float(trajedi_config.get("DIV_WEIGHT", 0.1))
    group_mm = int(trajedi_config.get("GROUP_MM", 1))
    group_mp = int(trajedi_config.get("GROUP_MP", 1))
    if group_mm < 0 or group_mp < 0 or group_mm + group_mp <= 0:
        raise ValueError("TrajeDi requires GROUP_MM/GROUP_MP to be non-negative with positive sum.")
    ...
    print("trajedi mm actors", mm_mask_flat.sum(), "mp actors", mp_mask_flat.sum())
    print("trajedi ego update actors", ego_actor_mask_flat.sum(), "partner update actors", partner_actor_mask_flat.sum())

    def _update_checkpoint(checkpoint_states, params, i):
        jax.debug.print("Saving TrajeDi checkpoint {i}", i=i)
    ...
    def train(
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_step_state, unused):
                ...
                info["trajedi_pair_partner_id_mean"] = jnp.full(
                    (model_config["NUM_ACTORS"],), partner_id_mean
                )
                info["trajedi_pair_is_mp"] = jnp.broadcast_to(
                    mp_mask_flat.astype(jnp.float32), (model_config["NUM_ACTORS"],)
                )
                info["trajedi_ego_update_mask"] = jnp.broadcast_to(
                    ego_actor_mask_flat.astype(jnp.float32), (model_config["NUM_ACTORS"],)
                )
                info["trajedi_partner_update_mask"] = jnp.broadcast_to(
            ...
            def _loss_fn(ego_params, partner_params, traj_batch, gae, targets):
                ...
                sim_mi_logits_all = jnp.where(
                    current_partner_mask[..., None],
                    partner_logits_all,
                    jax.lax.stop_gradient(partner_logits_all),
                )
                all_partner_probs = jax.nn.softmax(sim_mi_logits_all, axis=-1)
                action_idx = traj_batch.action[jnp.newaxis, ..., None]
                chosen_action_probs = jnp.take_along_axis(
                    all_partner_probs, action_idx, axis=-1
                ).squeeze(-1)
                current_action_prob = _select_by_partner_id(
                    chosen_action_probs, traj_batch.partner_id
                )
                mean_action_prob = jnp.mean(chosen_action_probs, axis=0)
                trajedi_sim_mi = jnp.log(
                    (current_action_prob + 1e-6) / (mean_action_prob + 1e-6)
                )
                trajedi_sim_mi = _safe_masked_mean(trajedi_sim_mi, partner_mask)
                partner_total = partner_ppo - trajedi_div_weight * trajedi_sim_mi
            ...
            metric["trajedi_population_size"] = jnp.array(population_size, dtype=jnp.float32)
            metric["trajedi_div_weight"] = jnp.array(trajedi_div_weight, dtype=jnp.float32)
            metric["trajedi_group_mm"] = jnp.array(group_mm, dtype=jnp.float32)
            metric["trajedi_group_mp"] = jnp.array(group_mp, dtype=jnp.float32)
            update_step += 1
