...
from overcooked_v2_experiments.ttac_v5_2_control_targets.models.abstract import ActorCriticBase
from overcooked_v2_experiments.ttac_v5_2_control_targets.models.model import get_actor_critic, initialize_carry
from overcooked_v2_experiments.ttac_v5_2_control_targets.utils.surrogate_heads import (
    apply_q_eta,
    expected_joint_q,
)
from overcooked_v2_experiments.ttac_v5_2_control_targets.utils.agreement_heads import (
...
EVAL_MODES = (
    ...
    "ttac_ctrl_random_nonbase_tv_gate",
    "ttac_ctrl_antistay_tv_gate",
    "ttac_ctrl_entropy_tv_gate",
    "ttac_wrong_history",
    "ttac_random_history",
...
class PPOPolicy(AbstractPolicy):
    ...
    def _ttac_loss(self, params, hstate, use_kl):
        ...
        if agreement_estimator_params is None:
            ...
            v7_return_score = jnp.array(0.0, dtype=jnp.float32)
            v7_tv_score = jnp.array(0.0, dtype=jnp.float32)
            v7_value_score = jnp.array(0.0, dtype=jnp.float32)
            ctrl_random_nonbase_loss = jnp.array(0.0, dtype=jnp.float32)
            ctrl_antistay_loss = jnp.array(0.0, dtype=jnp.float32)
            ctrl_entropy_loss = jnp.array(0.0, dtype=jnp.float32)
        else:
            latest_idx = (hstate.cursor - 1) % self.history_len
            order = (hstate.cursor - self.history_len + jnp.arange(self.history_len)) % self.history_len
            chron_actions = hstate.partner_action_buffer[order]
            ...
            v7_tv_threshold = jnp.asarray(
                self.model_config.get("TTAC_V7_TV_THRESHOLD", 0.03), dtype=jnp.float32
            )
            v7_tv_slope = jnp.asarray(
                self.model_config.get("TTAC_V7_TV_SLOPE", 25.0), dtype=jnp.float32
            )
            v7_value_margin = jnp.asarray(
                self.model_config.get("TTAC_V7_VALUE_MARGIN", 0.0), dtype=jnp.float32
            )
            v7_value_temp = jnp.asarray(
                self.model_config.get("TTAC_V7_VALUE_TEMP", 5.0), dtype=jnp.float32
            )
            v7_tv_score = jax.nn.sigmoid(
                (v5_2_latest_target_base_tv - v7_tv_threshold) * v7_tv_slope
            )
            v7_value_score = jax.nn.sigmoid(
                (value_center_v5 - v5_2_latest_value - v7_value_margin)
                / jnp.maximum(v7_value_temp, 1e-6)
            )
            v7_return_score = latest_valid * v7_tv_score * v7_value_score

            selected_ego_logits = jnp.take(query_ego_logits, v5_2_query_pos, axis=0)
            selected_ego_log_probs = jax.nn.log_softmax(selected_ego_logits, axis=-1)
            selected_ego_probs = jax.nn.softmax(selected_ego_logits, axis=-1)
            selected_base_probs = jnp.take(query_base_probs, v5_2_query_pos, axis=0)
            selected_base_argmax = jnp.argmax(selected_base_probs, axis=-1).astype(jnp.int32)
            nonbase_offset = (
                hstate.global_step % jnp.asarray(max(self.action_dim - 1, 1), dtype=jnp.int32)
            ) + jnp.asarray(1, dtype=jnp.int32)
            random_nonbase_action = (selected_base_argmax + nonbase_offset) % self.action_dim
            random_nonbase_target = jax.nn.one_hot(random_nonbase_action, self.action_dim)
            ctrl_random_nonbase_loss = -jnp.sum(
                jax.lax.stop_gradient(random_nonbase_target) * selected_ego_log_probs
            )
            stay_action = jnp.asarray(
                int(self.model_config.get("TTAC_CTRL_STAY_ACTION", 4)), dtype=jnp.int32
            )
            anti_stay_mask = 1.0 - jax.nn.one_hot(stay_action, self.action_dim)
            anti_stay_target = anti_stay_mask / jnp.maximum(
                anti_stay_mask.sum(), jnp.asarray(1.0, dtype=jnp.float32)
            )
            ctrl_antistay_loss = -jnp.sum(
                jax.lax.stop_gradient(anti_stay_target) * selected_ego_log_probs
            )
            ctrl_entropy_loss = jnp.sum(
                selected_ego_probs * jax.nn.log_softmax(selected_ego_logits, axis=-1)
            )
        ...
        v7_correction_coef = jnp.asarray(
            self.model_config.get("TTAC_V7_CORRECTION_COEF", 1.0), dtype=jnp.float32
        )
        v7_support_coef = jnp.asarray(
            self.model_config.get("TTAC_V7_SUPPORT_COEF", 0.0), dtype=jnp.float32
        )
        ...
        if self.eval_mode == "ttac_projected_history":
            ...
            mode_loss_gate = v7_return_score
            agreement_loss = (
                v5_agreement_coef * v7_correction_coef * v5_agreement_loss
                + v7_support_coef * v3_support_aw_loss
            )
        elif self.eval_mode in (
            "ttac_v7_return_weighted_tv_gate",
            "ttac_v7_return_weighted_tv_gate_wrong_history",
            ...
            mode_loss_gate = v7_return_score * v5_2_tv_gate
            agreement_loss = (
                v5_agreement_coef * v7_correction_coef * v5_agreement_loss
                + v7_support_coef * v3_support_aw_loss
            )
            ...
            mode_loss_gate = v7_return_score * v5_2_value_tv_gate
            agreement_loss = (
                v5_agreement_coef * v7_correction_coef * v5_agreement_loss
                + v7_support_coef * v3_support_aw_loss
            )
        elif self.eval_mode == "ttac_ctrl_random_nonbase_tv_gate":
            mode_loss_gate = v5_2_tv_gate
            agreement_loss = v5_agreement_coef * ctrl_random_nonbase_loss
        elif self.eval_mode == "ttac_ctrl_antistay_tv_gate":
            mode_loss_gate = v5_2_tv_gate
            agreement_loss = v5_agreement_coef * ctrl_antistay_loss
        elif self.eval_mode == "ttac_ctrl_entropy_tv_gate":
            mode_loss_gate = v5_2_tv_gate
            agreement_loss = v5_agreement_coef * ctrl_entropy_loss
        ...
        aux = {
            ...
            "v7_return_score": v7_return_score,
            "v7_tv_score": v7_tv_score,
            "v7_value_score": v7_value_score,
            "ctrl_random_nonbase_loss": ctrl_random_nonbase_loss,
            "ctrl_antistay_loss": ctrl_antistay_loss,
            "ctrl_entropy_loss": ctrl_entropy_loss,
        }
        return loss, aux
    ...
    def update_after_step(self, hstate, partner_obs, partner_action, done):
        ...
        should_update = jnp.asarray(
            ...
            or self.eval_mode.startswith("ttac_v7_")
            or self.eval_mode.startswith("ttac_ctrl_")
        )
...
def policy_checkoints_to_functional_policy_pairing(
    ...
    return FunctionalPolicyPairing(
        tuple(policies),
        backend="ttac_v5_2_control_targets",
        config=config,
    )
