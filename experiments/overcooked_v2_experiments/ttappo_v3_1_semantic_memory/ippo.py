...
def _apply_training_network(network, params, hstate, ac_in):
...
def _tree_expand_batch_dim(tree):
    return jax.tree_util.tree_map(lambda x: x[jnp.newaxis, ...], tree)


def _tree_squeeze_batch_dim(tree):
    return jax.tree_util.tree_map(lambda x: x.squeeze(axis=0), tree)
    ...
    def train(
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_step_state, unused):
                ...
                hstate, pi, value, aux = _apply_training_network(
                ...
                partner_action = (
                    action.reshape((env.num_agents, model_config["NUM_ENVS"]))[::-1]
                    .reshape(-1)
                )
                ...
                hstate = network.apply(
                    ...
                    jnp.ones_like(done_batch, dtype=jnp.bool_),
            ...
            _, _, last_val, _ = _apply_training_network(
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    minibatch, semantic_rng = batch_info
                    ...
                    def _loss_fn(
                        ...
                        if hstate is not None:
                            hstate = _tree_squeeze_batch_dim(hstate)
                        ...
                        def _masked_mean(x, mask):
                            ...
                            return jnp.sum(x * mask_f) / jnp.maximum(
                                jnp.sum(mask_f),
                                1.0,
                            )

                        true_partner_action = traj_batch.partner_action.astype(jnp.int32)
                        train_mask_array = jnp.broadcast_to(
                            jnp.asarray(train_mask, dtype=jnp.bool_),
                            true_partner_action.shape,
                        )

                        # Positive forward with the true partner action.
                        _, pi, value, aux = _apply_training_network(
                            network,
                            params,
                            hstate,
                            (
                                ...
                                traj_batch.done,
                                true_partner_action,
                                jnp.ones_like(true_partner_action, dtype=jnp.bool_),
                            ),
                        )

                        log_prob = pi.log_prob(traj_batch.action)
                        partner_logits = aux["partner_logits"]
                        partner_loss = optax.softmax_cross_entropy_with_integer_labels(
                            partner_logits,
                            true_partner_action,
                        ).mean(where=train_mask)
                        partner_pred = jnp.argmax(partner_logits, axis=-1)
                        partner_acc = (
                            (partner_pred == true_partner_action)
                            .astype(jnp.float32)
                            .mean(where=train_mask)
                        )

                        action_dim = partner_logits.shape[-1]
                        wrong_partner_action = traj_batch.action.astype(jnp.int32)
                        random_base_action = jax.random.randint(
                            semantic_rng,
                            shape=true_partner_action.shape,
                            minval=0,
                            maxval=action_dim - 1,
                        )
                        random_partner_action = random_base_action + (
                            random_base_action >= true_partner_action
                        ).astype(random_base_action.dtype)

                        # Negative forwards with corrupted partner actions.
                        _, _, _, aux_wrong = _apply_training_network(
                            network,
                            params,
                            hstate,
                            (
                                traj_batch.obs,
                                traj_batch.done,
                                wrong_partner_action,
                                jnp.ones_like(wrong_partner_action, dtype=jnp.bool_),
                            ),
                        )
                        _, _, _, aux_rand = _apply_training_network(
                            network,
                            params,
                            hstate,
                            (
                                traj_batch.obs,
                                traj_batch.done,
                                random_partner_action,
                                jnp.ones_like(random_partner_action, dtype=jnp.bool_),
                            ),
                        )

                        ce_pos = optax.softmax_cross_entropy_with_integer_labels(
                            partner_logits,
                            true_partner_action,
                        )
                        ce_wrong = optax.softmax_cross_entropy_with_integer_labels(
                            aux_wrong["partner_logits"],
                            true_partner_action,
                        )
                        ce_rand = optax.softmax_cross_entropy_with_integer_labels(
                            aux_rand["partner_logits"],
                            true_partner_action,
                        )
                        semantic_margin = model_config["SEMANTIC_MEMORY_MARGIN"]
                        wrong_mask = jnp.logical_and(
                            train_mask_array,
                            wrong_partner_action != true_partner_action,
                        )
                        semantic_loss_wrong = jnp.maximum(
                            0.0,
                            semantic_margin + ce_pos - ce_wrong,
                        )
                        semantic_loss_rand = jnp.maximum(
                            0.0,
                            semantic_margin + ce_pos - ce_rand,
                        )
                        semantic_loss = 0.5 * (
                            _masked_mean(semantic_loss_wrong, wrong_mask)
                            + _masked_mean(semantic_loss_rand, train_mask_array)
                        )
                        semantic_gap_wrong = _masked_mean(
                            ce_wrong - ce_pos,
                            wrong_mask,
                        )
                        semantic_gap_rand = _masked_mean(
                            ce_rand - ce_pos,
                            train_mask_array,
                        )
                        ce_pos_mean = _masked_mean(ce_pos, train_mask_array)
                        ce_wrong_mean = _masked_mean(ce_wrong, wrong_mask)
                        ce_rand_mean = _masked_mean(ce_rand, train_mask_array)
                        ...
                        total_loss = (
                            ...
                            + model_config["TTT_PARTNER_LOSS_COEF"] * partner_loss
                            + model_config["SEMANTIC_MEMORY_LOSS_COEF"] * semantic_loss
                ...
                hstate = init_hstate
                if hstate is not None:
                    hstate = _tree_expand_batch_dim(hstate)
                ...
                perm_rng, minibatch_rng = jax.random.split(_rng)
                ...
                minibatch_rngs = jax.random.split(
                    minibatch_rng,
                    model_config["NUM_MINIBATCHES"],
                )
            ...
            metric["ratio"] = ratio
            metric["partner_pred_loss"] = partner_loss
            metric["partner_pred_acc"] = partner_acc
            metric["semantic_memory_loss"] = semantic_loss
            metric["semantic_ce_pos"] = semantic_ce_pos
            metric["semantic_ce_wrong"] = semantic_ce_wrong
            metric["semantic_ce_rand"] = semantic_ce_rand
            metric["semantic_gap_wrong"] = semantic_gap_wrong
            metric["semantic_gap_rand"] = semantic_gap_rand
