...
class ActorCriticCNN(ActorCriticBase):
    ...
    def _extract_inputs(self, x):
        if len(x) == 2:
            ...
            raise ValueError(
                "ActorCriticCNN expects input as (obs, done) or "
                "(obs, done, partner_action, update_mask)"
            )
        return obs, done, partner_action, update_mask
    ...
    def __call__(self, hidden: PartnerMemoryCarry, x):
        obs, done, partner_action, update_mask = self._extract_inputs(x)
        activation = self._activation()
        ...
        feature_mod = gamma * feature_z + beta
