...
from overcooked_v2_experiments.ttac_v6_support_refinement.models.abstract import ActorCriticBase
from overcooked_v2_experiments.ttac_v6_support_refinement.models.model import get_actor_critic, initialize_carry
from overcooked_v2_experiments.ttac_v6_support_refinement.utils.surrogate_heads import (
    apply_q_eta,
    expected_joint_q,
)
...
EVAL_MODES = (
    ...
    "ttac_v6_support_mixed",
    "ttac_v6_support_mixed_wrong_history",
    "ttac_v6_support_mixed_random_history",
    "ttac_v6_support_mixed_delayed_history",
    "ttac_v6_support_soft",
    "ttac_v6_support_soft_wrong_history",
    "ttac_v6_support_soft_random_history",
    "ttac_v6_support_soft_delayed_history",
    "ttac_v6_support_hard",
    "ttac_v6_support_hard_wrong_history",
    "ttac_v6_support_hard_random_history",
    "ttac_v6_support_hard_delayed_history",
    "ttac_v6_support_tv_value",
    "ttac_v6_support_tv_value_wrong_history",
    "ttac_v6_support_tv_value_random_history",
    "ttac_v6_support_tv_value_delayed_history",
    "ttac_v6_agreement_amp",
    "ttac_v6_agreement_amp_wrong_history",
    "ttac_v6_agreement_amp_random_history",
    "ttac_v6_agreement_amp_delayed_history",
    "ttac_v6_agreement_amp_tv_gate",
    "ttac_v6_agreement_amp_tv_gate_wrong_history",
    "ttac_v6_agreement_amp_tv_gate_random_history",
    "ttac_v6_agreement_amp_tv_gate_delayed_history",
    "ttac_v6_agreement_amp_soft_gate",
    "ttac_v6_agreement_amp_soft_gate_wrong_history",
    "ttac_v6_agreement_amp_soft_gate_random_history",
    "ttac_v6_agreement_amp_soft_gate_delayed_history",
    "ttac_wrong_history",
    "ttac_random_history",
...
class PPOPolicy(AbstractPolicy):
    ...
    def _ttac_loss(self, params, hstate, use_kl):
        ...
        if agreement_estimator_params is None:
            ...
            latest_valid = jnp.array(0.0, dtype=jnp.float32)
            v6_support_mixed_loss = jnp.array(0.0, dtype=jnp.float32)
            v6_support_soft_gate = jnp.array(0.0, dtype=jnp.float32)
            v6_support_hard_gate = jnp.array(0.0, dtype=jnp.float32)
            v6_support_tv_value_gate = jnp.array(0.0, dtype=jnp.float32)
            v6_support_beta = jnp.array(0.0, dtype=jnp.float32)
            v6_support_confidence = jnp.array(0.0, dtype=jnp.float32)
            v6_support_value_gap = jnp.array(0.0, dtype=jnp.float32)
            v6_agreement_amp = jnp.array(1.0, dtype=jnp.float32)
        else:
            latest_idx = (hstate.cursor - 1) % self.history_len
            order = (hstate.cursor - self.history_len + jnp.arange(self.history_len)) % self.history_len
            chron_actions = hstate.partner_action_buffer[order]
            ...
            # v6 support refinement: use the agreement estimator as a local
            # refinement proposal only when base policy support looks weak.
            selected_estimator_probs = jnp.take(estimator_probs, v5_2_query_pos, axis=0)
            selected_base_probs = jnp.take(query_base_probs, v5_2_query_pos, axis=0)
            selected_log_probs = jnp.take(query_ego_log_probs, v5_2_query_pos, axis=0)
            action_log_dim = jnp.log(jnp.asarray(self.action_dim, dtype=jnp.float32))
            v6_support_confidence = jnp.clip(
                1.0 - v5_2_latest_entropy / jnp.maximum(action_log_dim, 1e-6),
                0.0,
                1.0,
            )
            v6_support_value_gap = value_center_v5 - v5_2_latest_value
            v6_value_positive = jnp.maximum(v6_support_value_gap, 0.0)
            v6_tv_threshold = jnp.asarray(
                self.model_config.get("TTAC_V6_TV_THRESHOLD", 0.03), dtype=jnp.float32
            )
            v6_value_margin = jnp.asarray(
                self.model_config.get("TTAC_V6_VALUE_MARGIN", 0.0), dtype=jnp.float32
            )
            v6_conf_threshold = jnp.asarray(
                self.model_config.get("TTAC_V6_CONF_THRESHOLD", 0.45), dtype=jnp.float32
            )
            v6_tv_slope = jnp.asarray(
                self.model_config.get("TTAC_V6_TV_SLOPE", 25.0), dtype=jnp.float32
            )
            v6_value_slope = jnp.asarray(
                self.model_config.get("TTAC_V6_VALUE_SLOPE", 1.0), dtype=jnp.float32
            )
            v6_conf_slope = jnp.asarray(
                self.model_config.get("TTAC_V6_CONF_SLOPE", 10.0), dtype=jnp.float32
            )
            v6_beta_scale = jnp.asarray(
                self.model_config.get("TTAC_V6_BETA_SCALE", 1.0), dtype=jnp.float32
            )
            v6_beta_max = jnp.asarray(
                self.model_config.get("TTAC_V6_BETA_MAX", 0.8), dtype=jnp.float32
            )
            v6_tv_soft = jax.nn.sigmoid(
                v6_tv_slope * (v5_2_latest_target_base_tv - v6_tv_threshold)
            )
            v6_value_soft = jax.nn.sigmoid(
                v6_value_slope * (v6_support_value_gap - v6_value_margin)
            )
            v6_conf_soft = jax.nn.sigmoid(
                v6_conf_slope * (v6_support_confidence - v6_conf_threshold)
            )
            v6_support_soft_gate = latest_valid * v6_tv_soft * v6_value_soft * v6_conf_soft
            v6_support_hard_gate = latest_valid * (
                (v5_2_latest_target_base_tv >= v6_tv_threshold)
                & (v6_support_value_gap >= v6_value_margin)
                & (v6_support_confidence >= v6_conf_threshold)
            ).astype(jnp.float32)
            v6_support_tv_value_gate = latest_valid * (
                (v5_2_latest_target_base_tv >= v6_tv_threshold)
                & (v6_support_value_gap >= v6_value_margin)
            ).astype(jnp.float32)
            v6_support_beta = jnp.clip(
                v6_beta_scale
                * v5_2_latest_target_base_tv
                * v6_value_positive
                * v6_support_confidence,
                0.0,
                v6_beta_max,
            )
            v6_support_target = jax.lax.stop_gradient(
                (1.0 - v6_support_beta) * selected_base_probs
                + v6_support_beta * selected_estimator_probs
            )
            v6_support_mixed_loss = -jnp.sum(
                v6_support_target * selected_log_probs, axis=-1
            )
            v6_amp_scale = jnp.asarray(
                self.model_config.get("TTAC_V6_AMP_SCALE", 2.0), dtype=jnp.float32
            )
            v6_amp_max = jnp.asarray(
                self.model_config.get("TTAC_V6_AMP_MAX", 2.0), dtype=jnp.float32
            )
            v6_agreement_amp = jnp.clip(
                1.0
                + v6_amp_scale
                * jnp.maximum(v5_2_latest_target_base_tv - v6_tv_threshold, 0.0)
                * v6_support_confidence,
                1.0,
                v6_amp_max,
            )
        ...
        if self.eval_mode == "ttac_projected_history":
            ...
            mode_loss_gate = latest_valid
            agreement_loss = v5_agreement_coef * v6_support_mixed_loss
        elif self.eval_mode in (
            "ttac_v6_support_soft",
            "ttac_v6_support_soft_wrong_history",
            "ttac_v6_support_soft_random_history",
            ...
            mode_loss_gate = v6_support_soft_gate
            agreement_loss = v5_agreement_coef * v6_support_mixed_loss
        elif self.eval_mode in (
            "ttac_v6_support_hard",
            "ttac_v6_support_hard_wrong_history",
            ...
            mode_loss_gate = v6_support_hard_gate
            agreement_loss = v5_agreement_coef * v6_support_mixed_loss
        elif self.eval_mode in (
            "ttac_v6_support_tv_value",
            "ttac_v6_support_tv_value_wrong_history",
            ...
            mode_loss_gate = v6_support_tv_value_gate
            agreement_loss = v5_agreement_coef * v6_support_mixed_loss
        elif self.eval_mode in (
            "ttac_v6_agreement_amp",
            "ttac_v6_agreement_amp_wrong_history",
            ...
            mode_loss_gate = latest_valid
            agreement_loss = v6_agreement_amp * v5_agreement_coef * v5_agreement_loss
        elif self.eval_mode in (
            "ttac_v6_agreement_amp_tv_gate",
            "ttac_v6_agreement_amp_tv_gate_wrong_history",
            "ttac_v6_agreement_amp_tv_gate_random_history",
        ...
        aux = {
            ...
            "v6_support_soft_gate": v6_support_soft_gate,
            "v6_support_hard_gate": v6_support_hard_gate,
            "v6_support_tv_value_gate": v6_support_tv_value_gate,
            "v6_support_beta": v6_support_beta,
            "v6_support_confidence": v6_support_confidence,
            "v6_support_value_gap": v6_support_value_gap,
            "v6_agreement_amp": v6_agreement_amp,
        }
        return loss, aux
    ...
    def update_after_step(self, hstate, partner_obs, partner_action, done):
        ...
        should_update = jnp.asarray(
            ...
            or self.eval_mode.startswith("ttac_v6_")
        )
...
def policy_checkoints_to_functional_policy_pairing(
    ...
    return FunctionalPolicyPairing(
        tuple(policies),
        backend="ttac_v6_support_refinement",
        config=config,
    )
...
def get_functional_rollout(
    ...
    def _ttac_v5_2_loss(params, state, eval_mode):
        ...
        if agreement_estimator_params is None:
            ...
            elif eval_mode.startswith("ttac_v6_"):
                _, selected_value, _ = _apply_batch_fn(
                    params, selected_obs[None, ...], 1.0
                )
                _, hist_values, _ = _apply_batch_fn(
                    params, state.ego_obs_buffer[order], 1.0
                )
                value_center = _masked_mean(hist_values, valid_chron.astype(jnp.float32))
                entropy = -jnp.sum(
                    estimator_probs * jnp.log(jnp.maximum(estimator_probs, 1e-6)),
                    axis=-1,
                )[0]
                action_log_dim = jnp.log(jnp.asarray(action_dim, dtype=jnp.float32))
                confidence = jnp.clip(
                    1.0 - entropy / jnp.maximum(action_log_dim, 1e-6),
                    0.0,
                    1.0,
                )
                value_gap = value_center - selected_value[0]
                value_positive = jnp.maximum(value_gap, 0.0)
                v6_tv_threshold = jnp.asarray(
                    model_config.get("TTAC_V6_TV_THRESHOLD", 0.03), dtype=jnp.float32
                )
                v6_value_margin = jnp.asarray(
                    model_config.get("TTAC_V6_VALUE_MARGIN", 0.0), dtype=jnp.float32
                )
                v6_conf_threshold = jnp.asarray(
                    model_config.get("TTAC_V6_CONF_THRESHOLD", 0.45), dtype=jnp.float32
                )
                v6_tv_slope = jnp.asarray(
                    model_config.get("TTAC_V6_TV_SLOPE", 25.0), dtype=jnp.float32
                )
                v6_value_slope = jnp.asarray(
                    model_config.get("TTAC_V6_VALUE_SLOPE", 1.0), dtype=jnp.float32
                )
                v6_conf_slope = jnp.asarray(
                    model_config.get("TTAC_V6_CONF_SLOPE", 10.0), dtype=jnp.float32
                )
                v6_beta_scale = jnp.asarray(
                    model_config.get("TTAC_V6_BETA_SCALE", 1.0), dtype=jnp.float32
                )
                v6_beta_max = jnp.asarray(
                    model_config.get("TTAC_V6_BETA_MAX", 0.8), dtype=jnp.float32
                )
                v6_amp_scale = jnp.asarray(
                    model_config.get("TTAC_V6_AMP_SCALE", 2.0), dtype=jnp.float32
                )
                v6_amp_max = jnp.asarray(
                    model_config.get("TTAC_V6_AMP_MAX", 2.0), dtype=jnp.float32
                )
                tv_soft = jax.nn.sigmoid(v6_tv_slope * (selected_tv - v6_tv_threshold))
                value_soft = jax.nn.sigmoid(
                    v6_value_slope * (value_gap - v6_value_margin)
                )
                conf_soft = jax.nn.sigmoid(
                    v6_conf_slope * (confidence - v6_conf_threshold)
                )
                soft_gate = selected_valid * tv_soft * value_soft * conf_soft
                hard_gate = selected_valid * (
                    (selected_tv >= v6_tv_threshold)
                    & (value_gap >= v6_value_margin)
                    & (confidence >= v6_conf_threshold)
                ).astype(jnp.float32)
                tv_value_gate = selected_valid * (
                    (selected_tv >= v6_tv_threshold)
                    & (value_gap >= v6_value_margin)
                ).astype(jnp.float32)
                beta = jnp.clip(
                    v6_beta_scale * selected_tv * value_positive * confidence,
                    0.0,
                    v6_beta_max,
                )
                mixed_target = jax.lax.stop_gradient(
                    (1.0 - beta) * selected_base_probs + beta * estimator_probs
                )
                selected_ce = -jnp.sum(mixed_target * selected_log_probs, axis=-1)[0]
                if eval_mode == "ttac_v6_support_soft":
                    mode_loss_gate = soft_gate
                elif eval_mode == "ttac_v6_support_hard":
                    mode_loss_gate = hard_gate
                elif eval_mode == "ttac_v6_support_tv_value":
                    mode_loss_gate = tv_value_gate
                elif eval_mode == "ttac_v6_agreement_amp_tv_gate":
                    mode_loss_gate = tv_gate
                elif eval_mode == "ttac_v6_agreement_amp_soft_gate":
                    mode_loss_gate = soft_gate
                else:
                    mode_loss_gate = selected_valid
                if eval_mode.startswith("ttac_v6_agreement_amp"):
                    amp = jnp.clip(
                        1.0
                        + v6_amp_scale
                        * jnp.maximum(selected_tv - v6_tv_threshold, 0.0)
                        * confidence,
                        1.0,
                        v6_amp_max,
                    )
                    selected_ce = amp * selected_ce
            else:
                mode_loss_gate = jnp.array(1.0, dtype=jnp.float32)
            agreement_loss = (
                jnp.asarray(
