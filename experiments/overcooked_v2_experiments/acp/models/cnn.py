...
class ActorCriticCNN(ActorCriticBase):
    def setup(self):
        ...
        convention_modes = int(self.config.get("CONVENTION_NUM_MODES", 0))
        if convention_modes > 0:
            self.convention_logits_layer = nn.Dense(
                convention_modes,
                kernel_init=orthogonal(0.01),
                bias_init=constant(0.0),
                name="convention_logits",
            )
    ...
    def __call__(self, hidden: PartnerMemoryCarry, x):
        ...
        if not bool(self.config.get("USE_PARTNER_HISTORY", True)):
            memory_seq = jnp.zeros_like(memory_seq)
            final_memory = hidden.partner_memory
            memory_update_rate = jnp.zeros_like(memory_update_rate)
        ...
        convention_modes = int(self.config.get("CONVENTION_NUM_MODES", 0))
        if convention_modes > 0:
            convention_logits = self.convention_logits_layer(memory_seq)
        else:
            convention_logits = jnp.zeros((*memory_seq.shape[:-1], 1), dtype=memory_seq.dtype)

        actor_hidden = self.actor_backbone_layer(feature_mod)
