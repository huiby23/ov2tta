...
from overcooked_v2_experiments.ppo.policy import PPOPolicy as PPODiagnosticsPolicy
from overcooked_v2_experiments.ppo.utils.store import (
    load_all_checkpoints as load_ppo_all_checkpoints,
)
from overcooked_v2_experiments.ppo.utils.visualize_ppo import (
...
from overcooked_v2_experiments.acp.policy import (
    AdaptivePPOPolicy as ACPAdaptivePolicy,
)
from overcooked_v2_experiments.acp.policy import PPOPolicy as ACPPolicy
from overcooked_v2_experiments.acp.utils.store import (
    load_all_checkpoints as load_acp_all_checkpoints,
)
from overcooked_v2_experiments.acp.utils.visualize_ppo import (
    visualize_ppo_policy as visualize_acp_run,
)
...
def load_policies(run_dir, backend="ppo", eval_mode="memory_off"):
    if backend == "ppo":
        ...
        all_checkpoints, config = load_acp_all_checkpoints(run_dir, final_only=True)
    else:
        raise ValueError(f"Unsupported backend: {backend}")
    ...
    for run_key in run_keys:
        ...
        if backend == "acp":
            if eval_mode == "memory_off":
                policy = ACPPolicy(checkpoint.params, config)
                policy.eval_mode = eval_mode
            else:
                policy = ACPAdaptivePolicy(checkpoint, config, eval_mode=eval_mode)
