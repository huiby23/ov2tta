...
from overcooked_v2_experiments.gamma_ttac.run import save_ppo_style_checkpoints, single_run
...
def main(config):
    ...
    run_name = f"gamma_ttac_{model_name}_ov2_{layout_name}_{avs_str}"
    if optional_prefix:
        run_name = f"{optional_prefix}_{run_name}"
    with wandb.init(entity=config["wandb"]["ENTITY"], project=config["wandb"]["PROJECT"], tags=["GAMMA-TTAC", "TTACv2", model_name, "OvercookedV2", "generated-partner"], config=config, mode=config["wandb"]["WANDB_MODE"], name=run_name) as run:
