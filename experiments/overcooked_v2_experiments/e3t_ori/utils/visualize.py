...
from overcooked_v2_experiments.e3t_ori.policy import E3TParams, policy_checkoints_to_policy_pairing
from overcooked_v2_experiments.e3t_ori.utils.store import load_all_checkpoints
...
def visualize_e3t_policy(
    ...
    env_kwargs = copy.deepcopy(config["env"]["ENV_KWARGS"]) | extra_env_kwargs
    ...
    if cross:
        ...
        for run_combination in run_combinations:
            ...
            key_name = "cross-" + "_".join(run_ids)
            cross_combinations[key_name] = PolicyPairing(*[policy_pairings[i] for i in run_combination])
        ...
        all_params = jax.tree_util.tree_map(
            ...
            all_params,
            is_leaf=lambda x: type(x) is E3TParams,
        )
        ...
        for second_level, second_level_runs in first_level_runs.items():
            for annotation, viz in second_level_runs.items():
                if not no_viz:
                    ...
                    imageio.mimsave(viz_dir / f"{annotation}.gif", viz.frame_seq, "GIF", duration=0.5)
                rows.append([first_level, second_level, annotation, viz.total_reward])
...
if __name__ == "__main__":
    ...
    viz_mode = {"sp": (not args.cross) or args.all, "cross": args.cross or args.all}
    extra_env_kwargs = {}
    if args.no_reset:
        extra_env_kwargs["random_reset"] = False
        extra_env_kwargs["op_ingredient_permutations"] = False

    for mode in [m for m, v in viz_mode.items() if v]:
        visualize_e3t_policy(
            Path(args.d),
            key_sp if mode == "sp" else key_cross,
            final_only=(not args.all_ckpt) or (mode == "cross"),
