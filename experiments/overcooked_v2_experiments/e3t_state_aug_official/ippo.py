...
from overcooked_v2_experiments.e3t_state_aug_official.models.abstract import ActorCriticBase
...
def _soft_update_human_policy(params, copy_coef):
    ...
    module_pairs = (
        ("human_policy_conv_initial", "policy_conv_initial"),
        ("human_policy_conv_0", "policy_conv_0"),
        ("human_policy_conv_1", "policy_conv_1"),
        ("human_policy_dense_0", "policy_dense_0"),
        ("human_policy_dense_1", "policy_dense_1"),
        ("human_policy_dense_2", "policy_dense_2"),
        ("human_actor_out", "actor_out"),
        ("human_critic_out", "critic_out"),
    )
...
def make_train(
    ...
    train_batch_size = model_config["NUM_STEPS"] * model_config["NUM_ENVS"]
    ...
    def train(
        ...
        def _update_step(runner_state, unused):
            ...
            # CALCULATE ADVANTAGE
            # Official E3T runner bootstraps GAE with last_values = 0.
            last_val = jnp.zeros((model_config["NUM_ACTORS"],), dtype=traj_batch.value.dtype)
            ...
            train_traj_batch = _flatten_time_actor(_select_train_side(traj_batch))
            train_advantages = _flatten_time_actor(
                jnp.take(advantages, train_actor_indices, axis=1)
            )
            train_targets = _flatten_time_actor(
                jnp.take(targets, train_actor_indices, axis=1)
            )
