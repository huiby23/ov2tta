...
from overcooked_v2_experiments.e3t_state_aug_official.policy import PPOPolicy as PPODiagnosticsPolicy
from overcooked_v2_experiments.e3t_state_aug_official.utils.store import (
    load_all_checkpoints as load_ppo_all_checkpoints,
)
from overcooked_v2_experiments.e3t_state_aug_official.utils.visualize_ppo import (
...
def summarize_method(
    ...
    if config["model"]["TYPE"] != "CNN":
        raise ValueError(
            f"{method_name} is configured as {config['model']['TYPE']}, expected CNN"
        )
