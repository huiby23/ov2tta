...
from overcooked_v2_experiments.ttappo_v2_temporal.policy import PPOParams, summarize_ttt_source_stats
from overcooked_v2_experiments.ttappo_v2_temporal.utils.store import load_all_checkpoints, store_checkpoint
from overcooked_v2_experiments.ttappo_v2_temporal.utils.utils import get_run_base_dir
from overcooked_v2_experiments.ttappo_v2_temporal.utils.visualize_ppo import visualize_ppo_policy
...
def single_run_with_viz(config):
    from overcooked_v2_experiments.ttappo_v2_temporal.run import single_run
        ...
        for run_num in range(num_runs):
            for checkpoint in range(num_checkpoints):
                ...
                if checkpoint == num_checkpoints - 1:
                    stat_key = jax.random.PRNGKey(config["SEED"] + run_num)
                    ttt_source_stats = summarize_ttt_source_stats(
                        params, config, stat_key
                    )
                ...
                store_checkpoint(
                    ...
                    ttt_source_stats=ttt_source_stats,
                )
...
def main(config):
    ...
    if config["TUNE"]:
        from overcooked_v2_experiments.ttappo_v2_temporal.tune import tune

        tune(config)
    elif "NUM_ITERATIONS" in config:
        from overcooked_v2_experiments.ttappo_v2_temporal.state_sample_run import state_sample_run
