...
from overcooked_v2_experiments.ttappo_v2_temporal.policy import PPOPolicy, PPOParams
...
def store_checkpoint(
    ...
    checkpoint = {
        "config": config,
        "params": params,
        "ttt_source_stats": ttt_source_stats or {},
    }
...
def load_checkpoint(run_dir, run_num, checkpoint):
    ...
    return ckpt["config"], ckpt["params"], ckpt.get("ttt_source_stats", {})
...
def load_all_checkpoints(run_dir, final_only=True, skip_initial=False):
    ...
    for run_num_dir in run_dir.iterdir():
        ...
        for checkpoint_dir in run_num_dir.iterdir():
            ...
            config, params, ttt_source_stats = load_checkpoint(run_dir, run_num, ckpt_id)
            # policy = PPOPolicy(params, config)
            # policy = PPOPolicy.from_checkpoint(params, config)

            policy = PPOParams(params=params, ttt_source_stats=ttt_source_stats or {})
