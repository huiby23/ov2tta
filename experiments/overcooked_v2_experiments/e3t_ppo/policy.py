...
class E3TPPOPolicy(AbstractPolicy):
    ...
    def __init__(self, params, config, stochastic=True, with_batching=False):
        ...
        self.params = params
        self.use_history_context = config["model"].get("USE_HISTORY_CONTEXT", False)
    ...
    def compute_action(self, obs, done, hstate, key, params=None):
        ...
        if not self.use_history_context:
        ...
        if self.with_batching:
            bootstrap_history_obs = bootstrap_history_obs.reshape(
                (1, flat_obs.shape[0], self.context_length) + self.obs_shape
            )
            bootstrap_history_actions = bootstrap_history_actions.reshape(
                (1, flat_obs.shape[0], self.context_length)
            )
            init_mask = hstate.initialized.reshape(
                (1, flat_obs.shape[0]) + (1,) * (bootstrap_history_obs.ndim - 2)
            )
            init_act_mask = hstate.initialized.reshape((1, flat_obs.shape[0], 1))
        else:
            bootstrap_history_obs = bootstrap_history_obs.reshape(
                (1, 1, self.context_length) + self.obs_shape
            )
            bootstrap_history_actions = bootstrap_history_actions.reshape(
                (1, 1, self.context_length)
            )
            init_mask = hstate.initialized.reshape(
                (1, 1) + (1,) * (bootstrap_history_obs.ndim - 2)
            )
    ...
    def update_after_step(self, hstate, partner_obs, partner_action, done):
        ...
        next_history_obs = jnp.concatenate(
            [
                hstate.history_obs[:, 1:],
                hstate.pending_obs[:, None, ...].astype(hstate.history_obs.dtype),
            ],
            axis=1,
        )
    ...
    def init_hstate(self, batch_size, key=None):
        ...
        return E3TPPOHState(
            ...
            history_obs=jnp.zeros(
                (batch_size, self.context_length) + self.obs_shape,
                dtype=jnp.float32,
            ),
            history_actions=jnp.full(
