...
from overcooked_v2_experiments.e3t_ppo.policy import E3TPPOParams
...
def store_checkpoint(config, params, run_num, checkpoint, final=False):
    ...
    save_args = orbax_utils.save_args_from_target(checkpoint_obj)
    orbax_checkpointer.save(checkpoint_dir, checkpoint_obj, save_args=save_args)
...
def load_all_checkpoints(run_dir, final_only=True, skip_initial=False):
    ...
    for run_num_dir in run_dir.iterdir():
        ...
        for checkpoint_dir in run_num_dir.iterdir():
            ...
            checkpoints[checkpoint_dir.name] = E3TPPOParams(params=params)
