...
JOB_ROOT="${JOB_ROOT:-logs/mappo_cnn_64_16_rerun}"
...
summarize_csv() {
  ...
  local csv_path="$2"
  if [[ ! -f "${csv_path}" ]]; then
    echo "[mappo-cnn-64-16] missing_csv name=${name} csv=${csv_path}" >&2
    return 1
  fi
  "${PYTHON}" - <<PY
...
print(f"[mappo-cnn-64-16] result name={name} SP={sp_mean:.4f} XP={xp_mean:.4f} SP_pair_std={sp_std:.4f} XP_pair_std={xp_std:.4f} csv={path}")
PY
}
...
eval_mappo() {
  ...
  if [[ -z "${run_dir}" || ! -d "${run_dir}" ]]; then
    echo "[mappo-cnn-64-16] missing_run_dir name=${name} run_dir=${run_dir}" >&2
    exit 2
  fi
  echo "[mappo-cnn-64-16] eval_start=$(date -Is) method=${name} run_dir=${run_dir} eval_seeds=${EVAL_SEEDS}"
  ...
  echo "[mappo-cnn-64-16] eval_done=$(date -Is) method=${name}"
}

train_standard() {
  local method="mappo_cnn_standard"
  local prefix="${PREFIX_BASE}_${method}_${JOB_TAG}"
  echo "[mappo-cnn-64-16] train_start=$(date -Is) method=${method} prefix=${prefix}"
  ...
  echo "[mappo-cnn-64-16] train_done=$(date -Is) method=${method} run_dir=${run_dir}"
  eval_mappo "${method}" "${run_dir}"
}

train_state_aug() {
  local method="mappo_cnn_state_aug"
  local prefix="${PREFIX_BASE}_${method}_${JOB_TAG}"
  echo "[mappo-cnn-64-16] train_start=$(date -Is) method=${method} prefix=${prefix} iterations=${NUM_ITERATIONS}"
...
pipeline() {
  ...
  export PREFIX_BASE="${PREFIX_BASE:-figure4_mappo_cnn_64_16_rerun}"
  ...
  print_repro_env
  echo "[mappo-cnn-64-16] launch_time=$(date -Is) host=$(hostname) root=${ROOT} cuda=${CUDA_VISIBLE_DEVICES:-unset}"
  echo "[mappo-cnn-64-16] seed=${SEED} num_seeds=${NUM_SEEDS} methods=${METHODS} layout=${LAYOUT}"
  echo "[mappo-cnn-64-16] scale total=${TOTAL_TIMESTEPS} rew_horizon=${REW_SHAPING_HORIZON} envs=${NUM_ENVS} steps=${NUM_STEPS} epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES} iterations=${NUM_ITERATIONS} eval_seeds=${EVAL_SEEDS}"
  for method in ${METHODS}; do
    case "${method}" in
      standard) train_standard ;;
      state_aug) train_state_aug ;;
      *) echo "[mappo-cnn-64-16] unknown method=${method}" >&2; exit 2 ;;
    esac
  done
  echo "[mappo-cnn-64-16] pipeline_done=$(date -Is)"
  rm -f "${PID_FILE}"
}
...
status() {
  if is_running; then
    local pid
    pid="$(cat "${PID_FILE}")"
    echo "[mappo-cnn-64-16] running pid=${pid}"
    ps -fp "${pid}" || true
    echo "[mappo-cnn-64-16] children:"
    pgrep -P "${pid}" -af || true
  else
    echo "[mappo-cnn-64-16] not running"
    [[ -f "${PID_FILE}" ]] && echo "[mappo-cnn-64-16] stale pid=$(cat "${PID_FILE}")"
  fi
  if [[ -f "${LATEST_LOG}" ]]; then
    local log
    log="$(cat "${LATEST_LOG}")"
    echo "[mappo-cnn-64-16] latest_log=${log}"
...
start() {
  ...
  local log="${JOB_ROOT}/mappo_cnn_64_16_rerun_${JOB_TAG}.log"
  printf "%s\n" "${log}" > "${LATEST_LOG}"
  nohup env JOB_TAG="${JOB_TAG}" CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" PYTHON="${PYTHON}" \
    ...
    NUM_ITERATIONS="${NUM_ITERATIONS:-}" EVAL_SEEDS="${EVAL_SEEDS:-}" EVAL_SEED="${EVAL_SEED:-42}" \
    bash "${BASH_SOURCE[0]}" pipeline > "${log}" 2>&1 < /dev/null &
  local pid=$!
  echo "${pid}" > "${PID_FILE}"
  echo "[mappo-cnn-64-16] started pid=${pid} log=${log}"
}
...
case "${1:-start}" in
  ...
  stop) if is_running; then kill "$(cat "${PID_FILE}")"; else echo "[mappo-cnn-64-16] not running"; fi ;;
