...
REPORT_DIR="${REPORT_DIR:-reports/ttac_v5_2_state_selection_${TS}/stage1_same_script}"
LOG_DIR="${LOG_DIR:-logs/ttac_v5_2_state_selection_${TS}/stage1_same_script}"
SEED="${SEED:-42}"
...
rows = []
for p in sorted(run.glob('reward_summary_cross_v5_2_stage1_*.csv')):
    mode = p.name.removeprefix('reward_summary_cross_v5_2_stage1_').removesuffix('.csv')
