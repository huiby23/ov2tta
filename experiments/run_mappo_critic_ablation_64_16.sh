...
JOB_ROOT="${JOB_ROOT:-logs/mappo_critic_ablation_64_16}"
...
summarize_csv() {
  ...
  local csv_path="$2"
  if [[ ! -f "${csv_path}" ]]; then
    echo "[mappo-critic-ablation] missing_csv name=${name} csv=${csv_path}" >&2
    return 1
  fi
  "${PYTHON}" - <<PY
...
print(f"[mappo-critic-ablation] result name={name} SP={sp_mean:.4f} XP={xp_mean:.4f} SP_pair_std={sp_std:.4f} XP_pair_std={xp_std:.4f} csv={path}")
PY
}
...
eval_mappo() {
  ...
  if [[ -z "${run_dir}" || ! -d "${run_dir}" ]]; then
    echo "[mappo-critic-ablation] missing_run_dir name=${name} run_dir=${run_dir}" >&2
    exit 2
  fi
  echo "[mappo-critic-ablation] eval_start=$(date -Is) method=${name} run_dir=${run_dir} eval_seeds=${EVAL_SEEDS}"
  ...
  echo "[mappo-critic-ablation] eval_done=$(date -Is) method=${name}"
}

train_state_aug_mode() {
  local critic_mode="$1"
  local method="mappo_rnn_state_aug_critic_${critic_mode}"
  local prefix="${PREFIX_BASE}_${method}_${JOB_TAG}"
  echo "[mappo-critic-ablation] train_start=$(date -Is) method=${method} critic_input=${critic_mode} prefix=${prefix} iterations=${NUM_ITERATIONS}"
  ...
  "${PYTHON}" experiments/overcooked_v2_experiments/mappo/main.py \
    ...
    +model.CRITIC_INPUT_MODE="${critic_mode}" \
  ...
  echo "[mappo-critic-ablation] train_done=$(date -Is) method=${method} run_dir=${run_dir}"
  eval_mappo "${method}" "${run_dir}"
}

pipeline() {
  export JOB_TAG="${JOB_TAG:-$(date +%Y%m%d-%H%M%S)}"
  export PREFIX_BASE="${PREFIX_BASE:-mappo_critic_ablation_64_16}"
  ...
  export METHODS="${METHODS:-state_aug_world_state state_aug_local_obs state_aug_zero_world_state}"
  ...
  print_repro_env
  echo "[mappo-critic-ablation] launch_time=$(date -Is) host=$(hostname) root=${ROOT} cuda=${CUDA_VISIBLE_DEVICES:-unset}"
  echo "[mappo-critic-ablation] seed=${SEED} num_seeds=${NUM_SEEDS} methods=${METHODS} layout=${LAYOUT}"
  echo "[mappo-critic-ablation] scale total=${TOTAL_TIMESTEPS} rew_horizon=${REW_SHAPING_HORIZON} envs=${NUM_ENVS} steps=${NUM_STEPS} epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES} iterations=${NUM_ITERATIONS} eval_seeds=${EVAL_SEEDS}"
  echo "[mappo-critic-ablation] hparams LR=${LR} ENT_COEF=${ENT_COEF} MAX_GRAD_NORM=${MAX_GRAD_NORM}"
  for method in ${METHODS}; do
    case "${method}" in
      state_aug_world_state) train_state_aug_mode world_state ;;
      state_aug_local_obs) train_state_aug_mode local_obs ;;
      state_aug_zero_world_state) train_state_aug_mode zero_world_state ;;
      *) echo "[mappo-critic-ablation] unknown method=${method}" >&2; exit 2 ;;
    esac
  done
  echo "[mappo-critic-ablation] pipeline_done=$(date -Is)"
  rm -f "${PID_FILE}"
}
...
status() {
  if is_running; then
    local pid
    pid="$(cat "${PID_FILE}")"
    echo "[mappo-critic-ablation] running pid=${pid}"
    ps -fp "${pid}" || true
    echo "[mappo-critic-ablation] children:"
    pgrep -P "${pid}" -af || true
  else
    echo "[mappo-critic-ablation] not running"
    [[ -f "${PID_FILE}" ]] && echo "[mappo-critic-ablation] stale pid=$(cat "${PID_FILE}")"
  fi
  if [[ -f "${LATEST_LOG}" ]]; then
    local log
    log="$(cat "${LATEST_LOG}")"
    echo "[mappo-critic-ablation] latest_log=${log}"
...
start() {
  ...
  local log="${JOB_ROOT}/mappo_critic_ablation_64_16_${JOB_TAG}.log"
  printf "%s\n" "${log}" > "${LATEST_LOG}"
  ...
  local pid=$!
  echo "${pid}" > "${PID_FILE}"
  echo "[mappo-critic-ablation] started pid=${pid} log=${log}"
}
...
case "${1:-start}" in
  ...
  stop) if is_running; then kill "$(cat "${PID_FILE}")"; else echo "[mappo-critic-ablation] not running"; fi ;;
