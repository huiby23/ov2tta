...
JOB_ROOT="${JOB_ROOT:-logs/mappo_cnn_rnn_hparams_state_aug_64_16}"
...
summarize_csv() {
  ...
  local csv_path="$2"
  if [[ ! -f "${csv_path}" ]]; then
    echo "[mappo-cnn-rnnhp] missing_csv name=${name} csv=${csv_path}" >&2
    return 1
  fi
  "${PYTHON}" - <<PY
...
print(f"[mappo-cnn-rnnhp] result name={name} SP={sp_mean:.4f} XP={xp_mean:.4f} SP_pair_std={sp_std:.4f} XP_pair_std={xp_std:.4f} csv={path}")
PY
}

pipeline() {
  export JOB_TAG="${JOB_TAG:-$(date +%Y%m%d-%H%M%S)}"
  export PREFIX_BASE="${PREFIX_BASE:-figure4_mappo_cnn_rnn_hparams_state_aug_64_16}"
  ...
  print_repro_env
  local method="mappo_cnn_state_aug_rnn_hparams"
  local prefix="${PREFIX_BASE}_${JOB_TAG}"
  echo "[mappo-cnn-rnnhp] launch_time=$(date -Is) host=$(hostname) root=${ROOT} cuda=${CUDA_VISIBLE_DEVICES:-unset}"
  echo "[mappo-cnn-rnnhp] method=${method} prefix=${prefix} seed=${SEED} num_seeds=${NUM_SEEDS} layout=${LAYOUT}"
  echo "[mappo-cnn-rnnhp] scale total=${TOTAL_TIMESTEPS} rew_horizon=${REW_SHAPING_HORIZON} envs=${NUM_ENVS} steps=${NUM_STEPS} epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES} iterations=${NUM_ITERATIONS} eval_seeds=${EVAL_SEEDS}"
  echo "[mappo-cnn-rnnhp] rnn_hparams LR=${LR} ENT_COEF=${ENT_COEF} MAX_GRAD_NORM=${MAX_GRAD_NORM}"
  ...
  echo "[mappo-cnn-rnnhp] train_done=$(date -Is) method=${method} run_dir=${run_dir}"
  if [[ -z "${run_dir}" || ! -d "${run_dir}" ]]; then
    echo "[mappo-cnn-rnnhp] missing_run_dir prefix=${prefix}" >&2
    exit 2
  fi

  echo "[mappo-cnn-rnnhp] eval_start=$(date -Is) method=${method} run_dir=${run_dir} eval_seeds=${EVAL_SEEDS}"
  ...
  "${PYTHON}" experiments/overcooked_v2_experiments/mappo/utils/visualize.py \
    ...
    --seed "${EVAL_SEED}" \
    --no_viz
  summarize_csv "${method}" "${run_dir}/reward_summary_cross.csv"
  echo "[mappo-cnn-rnnhp] pipeline_done=$(date -Is)"
  rm -f "${PID_FILE}"
}
...
status() {
  if is_running; then
    local pid
    pid="$(cat "${PID_FILE}")"
    echo "[mappo-cnn-rnnhp] running pid=${pid}"
    ps -fp "${pid}" || true
    echo "[mappo-cnn-rnnhp] children:"
    pgrep -P "${pid}" -af || true
  else
    echo "[mappo-cnn-rnnhp] not running"
    [[ -f "${PID_FILE}" ]] && echo "[mappo-cnn-rnnhp] stale pid=$(cat "${PID_FILE}")"
  fi
  if [[ -f "${LATEST_LOG}" ]]; then
    local log
    log="$(cat "${LATEST_LOG}")"
    echo "[mappo-cnn-rnnhp] latest_log=${log}"
...
start() {
  ...
  local log="${JOB_ROOT}/mappo_cnn_rnn_hparams_state_aug_64_16_${JOB_TAG}.log"
  printf "%s\n" "${log}" > "${LATEST_LOG}"
  nohup env JOB_TAG="${JOB_TAG}" CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" PYTHON="${PYTHON}" \
    PREFIX_BASE="${PREFIX_BASE:-}" LAYOUT="${LAYOUT:-}" SEED="${SEED:-}" \
  ...
  local pid=$!
  echo "${pid}" > "${PID_FILE}"
  echo "[mappo-cnn-rnnhp] started pid=${pid} log=${log}"
}
...
case "${1:-start}" in
  ...
  stop) if is_running; then kill "$(cat "${PID_FILE}")"; else echo "[mappo-cnn-rnnhp] not running"; fi ;;
