...
TS="${TS:-$(date +%Y%m%d_%H%M%S_factorial)}"
...
LOG_DIR="${LOG_DIR:-logs/ttac_v2_factorial_${TS}}"
REPORT_DIR="${REPORT_DIR:-reports/ttac_v2_factorial_${TS}}"
...
run_train() {
  ...
  log "TRAIN_START tag=${tag} surrogate=${surrogate} partner=${partner_mode} prefix=${prefix}"
...
run_eval() {
  local tag="$1" run_dir="$2" eval_mode="$3" split="$4"
  local cross_args=()
  local summary="sp"
  if [ "${split}" = "cross" ]; then
    cross_args=(--cross)
...
for tag_file in sorted(report.glob("run_dir_*.txt")):
    ...
    for csv in sorted(run_dir.glob("reward_summary_*.csv")):
        ...
        mode = name.split(f"{tag}_", 1)[1].rsplit("_" + ("cross" if split == "XP" else "sp"), 1)[0]
        row = {
            "tag": tag,
            "mode": mode,
            "split": split,
...
if len(out):
    ...
    out.to_csv(report / "factorial_eval_summary.csv", index=False)
    try:
        table = out.to_markdown(index=False)
    except Exception:
        table = out.to_csv(index=False)
    (report / "factorial_eval_summary.md").write_text(
        "# TTACv2 factorial missing-cell full eval\n\n"
        f"report_dir: `{report}`\n\n" + table + "\n"
    )
...
run_experiment() {
  local tag="$1" surrogate="$2" partner_mode="$3"
  run_train "${tag}" "${surrogate}" "${partner_mode}"
  local run_dir
...
run_experiment selfplay_pc projected_confident selfplay
run_experiment frozen_mep_aw advantage_weighted frozen_mep
log "PIPELINE_DONE"
