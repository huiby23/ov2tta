...
REPORT_DIR="${REPORT_DIR:-reports/ttac_semantic_best_full_eval_${TS}}"
LOG_DIR="${LOG_DIR:-logs/ttac_semantic_best_full_eval_${TS}}"
SEED="${SEED:-42}"
...
if [ -z "${SWEEP_REPORT_DIR}" ]; then
  SWEEP_REPORT_DIR="$(find reports -maxdepth 1 -type d -name 'ttac_semantic_surrogate_sweep_*' | sort | tail -n 1)"
fi
