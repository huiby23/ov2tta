...
JOB_ROOT="${JOB_ROOT:-logs/official_fcp_256_64_30m_10learners}"
...
summarize_cross() {
  ...
  local csv_path="$2"
  if [[ ! -f "${csv_path}" ]]; then
    echo "[official-fcp-256-64-30m] missing_csv name=${name} csv=${csv_path}" >&2
    return 1
  fi
...
print(f"[official-fcp-256-64-30m] result name={name} SP={sp_mean:.4f} XP={xp_mean:.4f} SP_pair_std={sp_std:.4f} XP_pair_std={xp_std:.4f} n_sp_pairs={len(sp)} n_xp_pairs={len(xp)} csv={path}")
PY
}
...
register_result() {
  local run_dir="$1"
  local result_file="$2"
  mkdir -p runs_by_config/counter_circuit_full_obs_256env_64mb_30M/fcp/rnn_official_10learners
  ln -sfn "${ROOT}/${run_dir}" runs_by_config/counter_circuit_full_obs_256env_64mb_30M/fcp/rnn_official_10learners/latest
  if [[ -f experiments/run_registry.tsv ]]; then
    if ! grep -Fq "fcp_rnn_official_10learners_256_64_30m" experiments/run_registry.tsv; then
      printf "counter_circuit_full_obs_256env_64mb_30M\tfcp\trnn_official_10learners\tfcp_rnn_official_10learners_256_64_30m\t%s\t%s\tofficial FCP flow: 10 groups x 8 SP policies, 256 env / 64 minibatch / 30M\n" "${run_dir}" "${result_file}" >> experiments/run_registry.tsv
    fi
  fi
}
...
train_sp_group() {
  ...
  if [[ -n "${existing}" && -d "${existing}/run_7/ckpt_final" ]]; then
    echo "[official-fcp-256-64-30m] sp_group_reuse group=${group} run_dir=${existing}"
    printf "%s\n" "${existing}" > "${JOB_ROOT}/sp_group_${group}.run_dir"
    return
  fi
  echo "[official-fcp-256-64-30m] sp_group_train_start=$(date -Is) group=${group} seed=${group_seed} prefix=${prefix}"
  ...
  if [[ -z "${run_dir}" || ! -d "${run_dir}/run_7/ckpt_final" ]]; then
    echo "[official-fcp-256-64-30m] sp_group_missing_checkpoint group=${group} run_dir=${run_dir}" >&2
    exit 1
  fi
  printf "%s\n" "${run_dir}" > "${JOB_ROOT}/sp_group_${group}.run_dir"
  echo "[official-fcp-256-64-30m] sp_group_train_done=$(date -Is) group=${group} run_dir=${run_dir}"
}
  ...
  for group in $(seq 0 $((FCP_GROUPS - 1))); do
    ...
    for policy in $(seq 0 $((POPULATION_SIZE - 1))); do
      ...
      if [[ ! -d "${src}/ckpt_final" ]]; then
        echo "[official-fcp-256-64-30m] missing_sp_ckpt group=${group} policy=${policy} src=${src}" >&2
        exit 1
      fi
      cp -a "${src}" "${dst}"
    done
  done
  echo "[official-fcp-256-64-30m] population_ready dir=${POP_DIR} groups=${FCP_GROUPS} size=${POPULATION_SIZE} time=$(date -Is)"
}
...
train_fcp() {
  ...
  if [[ -n "${existing}" && -d "${existing}/run_9/ckpt_final" ]]; then
    echo "[official-fcp-256-64-30m] fcp_reuse run_dir=${existing}"
    printf "%s\n" "${existing}" > "${JOB_ROOT}/fcp.run_dir"
    return
  fi
  echo "[official-fcp-256-64-30m] fcp_train_start=$(date -Is) prefix=${FCP_PREFIX} pop=${POP_DIR}"
  PYTHONUNBUFFERED=1 PYTHONPATH="${ROOT}/experiments:${ROOT}/JaxMARL:${PYTHONPATH:-}" \
  XLA_PYTHON_CLIENT_PREALLOCATE="${XLA_PYTHON_CLIENT_PREALLOCATE:-false}" \
  "${PYTHON}" -m overcooked_v2_experiments.ppo.main \
    +experiment=rnn-fcp \
  ...
  if [[ -z "${run_dir}" || ! -d "${run_dir}/run_9/ckpt_final" ]]; then
    echo "[official-fcp-256-64-30m] fcp_missing_checkpoint run_dir=${run_dir}" >&2
    exit 1
  fi
  printf "%s\n" "${run_dir}" > "${JOB_ROOT}/fcp.run_dir"
  echo "[official-fcp-256-64-30m] fcp_train_done=$(date -Is) run_dir=${run_dir}"
}

evaluate_fcp() {
  local run_dir
  run_dir="$(cat "${JOB_ROOT}/fcp.run_dir")"
  echo "[official-fcp-256-64-30m] eval_start=$(date -Is) run_dir=${run_dir} eval_seeds=${EVAL_SEEDS}"
  PYTHONUNBUFFERED=1 PYTHONPATH="${ROOT}/experiments:${ROOT}/JaxMARL:${PYTHONPATH:-}" \
  XLA_PYTHON_CLIENT_PREALLOCATE="${XLA_PYTHON_CLIENT_PREALLOCATE:-false}" \
  "${PYTHON}" experiments/overcooked_v2_experiments/ppo/utils/visualize_ppo.py \
    ...
    --seed "${EVAL_SEED}" \
    --no_viz
  summarize_cross "fcp_rnn_official_10learners_256_64_30m" "${run_dir}/reward_summary_cross.csv"
  register_result "${run_dir}" "reward_summary_cross.csv"
  echo "[official-fcp-256-64-30m] eval_done=$(date -Is) run_dir=${run_dir}"
}
...
pipeline() {
  ...
  export NUM_MINIBATCHES="${NUM_MINIBATCHES:-64}"
  export POP_NUM_CHECKPOINTS="${POP_NUM_CHECKPOINTS:-3}"
  export FCP_NUM_CHECKPOINTS="${FCP_NUM_CHECKPOINTS:-3}"
  ...
  export POP_PREFIX="${POP_PREFIX:-official_fcp256_64_30m_sp_pop_${JOB_TAG}}"
  export FCP_PREFIX="${FCP_PREFIX:-official_fcp256_64_30m_rnn_10learners_${JOB_TAG}}"
  export POP_DIR="${POP_DIR:-fcp_populations/official_fcp256_64_30m_counter_circuit_10x8_${JOB_TAG}}"
  export PYTHONPATH="${ROOT}/experiments:${ROOT}/JaxMARL:${PYTHONPATH:-}"
  export XLA_PYTHON_CLIENT_PREALLOCATE="${XLA_PYTHON_CLIENT_PREALLOCATE:-false}"

  print_repro_env
  echo "[official-fcp-256-64-30m] pipeline_start=$(date -Is) host=$(hostname) cuda=${CUDA_VISIBLE_DEVICES:-unset}"
  echo "[official-fcp-256-64-30m] layout=${LAYOUT} seed=${SEED} groups=${FCP_GROUPS} pop_size=${POPULATION_SIZE} seed_stride=${SEED_STRIDE}"
  echo "[official-fcp-256-64-30m] scale total=${TOTAL_TIMESTEPS} rew_horizon=${REW_SHAPING_HORIZON} envs=${NUM_ENVS} steps=${NUM_STEPS} epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES} eval_seeds=${EVAL_SEEDS}"
  echo "[official-fcp-256-64-30m] prefixes pop=${POP_PREFIX} fcp=${FCP_PREFIX} pop_dir=${POP_DIR}"
  ...
  train_fcp
  evaluate_fcp
  echo "[official-fcp-256-64-30m] pipeline_done=$(date -Is)"
  rm -f "${PID_FILE}"
}
...
status() {
  if is_running; then
    local pid
    pid="$(cat "${PID_FILE}")"
    echo "[official-fcp-256-64-30m] running pid=${pid}"
    ps -fp "${pid}" || true
    echo "[official-fcp-256-64-30m] children:"
    pgrep -P "${pid}" -af || true
  else
    echo "[official-fcp-256-64-30m] not running"
    [[ -f "${PID_FILE}" ]] && echo "[official-fcp-256-64-30m] stale pid=$(cat "${PID_FILE}")"
  fi
  if [[ -f "${LATEST_LOG}" ]]; then
    local log
    log="$(cat "${LATEST_LOG}")"
    echo "[official-fcp-256-64-30m] latest_log=${log}"
...
start() {
  ...
  local log="${JOB_ROOT}/official_fcp_256_64_30m_10learners_${tag}.log"
  printf "%s\n" "${log}" > "${LATEST_LOG}"
  ...
  local pid=$!
  echo "${pid}" > "${PID_FILE}"
  echo "[official-fcp-256-64-30m] started pid=${pid} log=${log}"
}
...
case "${1:-start}" in
  ...
  stop) if is_running; then kill "$(cat "${PID_FILE}")"; else echo "[official-fcp-256-64-30m] not running"; fi ;;
