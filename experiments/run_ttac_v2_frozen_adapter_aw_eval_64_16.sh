...
TS="${TS:-$(date +%Y%m%d_%H%M%S_frozen_adapter)}"
...
LOG_DIR="${LOG_DIR:-logs/ttac_v2_frozen_adapter_aw_eval_${TS}}"
REPORT_DIR="${REPORT_DIR:-reports/ttac_v2_frozen_adapter_aw_eval_${TS}}"
...
run_train() {
  ...
  local prefix="ttac_v2_frozen_adapter_${phase}_state_aug_64_16_10M_seed42_10seeds_${TS}"
  ...
  "${PYTHON}" -m overcooked_v2_experiments.ttac_v2.main \
    ...
    model.TTAC_ADAPTER_SCALE="${TTAC_ADAPTER_SCALE}" model.TTAC_ZERO_INIT_ADAPTER_UP=true model.TTAC_TRAIN_ADAPTER_READOUT_SCALE=0.0 \
    model.TTAC_AGREEMENT_COEF=0.0 model.TTAC_TRAIN_KL_COEF=0.0 model.TTAC_TRAIN_SURROGATE=ce \
...
if len(out):
    ...
    (report / "full_eval_summary.md").write_text(
        "# TTACv2 frozen-train-adapter + AW online eval\n\n"
        f"run_dir: `{run_dir}`\n\n"
        + table
        + "\n"
    )
