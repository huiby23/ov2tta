...
REPORT_DIR="${REPORT_DIR:-reports/ttac_v5_2_control_targets_${TS}}"
LOG_DIR="${LOG_DIR:-logs/ttac_v5_2_control_targets_${TS}}"
...
TTAC_CTRL_STAY_ACTION="${TTAC_CTRL_STAY_ACTION:-4}"
TAG_PREFIX="${TAG_PREFIX:-v5_2_control_targets_${TS}}"
MODES="${MODES:-base_no_test_adapt ttac_v5_2_latest ttac_v5_2_tv_gate ttac_ctrl_random_nonbase_tv_gate ttac_ctrl_antistay_tv_gate ttac_ctrl_entropy_tv_gate}"
...
TTAC_CTRL_STAY_ACTION=${TTAC_CTRL_STAY_ACTION}
...
run_eval() {
  ...
  "${PYTHON}" experiments/overcooked_v2_experiments/ttac_v5_2_control_targets/utils/visualize_ppo.py \
    --d "${RUN_DIR}" \
    --seed "${SEED}" \
...
with (report / "control_targets_pair20_summary.csv").open("w", newline="") as f:
...
md = ["# TTAC v5.2 control-target pair20", ""]
...
(report / "control_targets_pair20_summary.md").write_text("\n".join(md) + "\n")
print("\n".join(md))
PY
}
