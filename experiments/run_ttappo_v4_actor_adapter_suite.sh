...
V3_PREFIX_BASE="${V3_PREFIX_BASE:-figure4_ttappo_v4_actor_adapter_${PIPELINE_TAG}}"
...
train_v3() {
  ...
  echo "[train] v4 actor-adapter prefix=${prefix} seed=${seed}"
  PYTHONPATH=experiments "$PYTHON_BIN" \
    experiments/overcooked_v2_experiments/ttappo_v4_actor_adapter/main.py \
    +experiment=cnn \
    +env=original \
    env.ENV_KWARGS.layout="${LAYOUT}" \
    SEED="${seed}" \
...
eval_v3_mode() {
  ...
  local mode="$2"
  local eval_seeds="$3"
  echo "[eval] v4 mode=${mode} run_dir=${run_dir}"
  PYTHONPATH=experiments "$PYTHON_BIN" \
    experiments/overcooked_v2_experiments/ttappo_v4_actor_adapter/utils/visualize_ppo.py \
    --d "${run_dir}" \
    --cross \
    --num_seeds "${eval_seeds}" \
    --no_viz \
...
run_v4_diagnostics() {
  ...
  if [[ "${DIAG_WORKERS}" -le 1 ]]; then
    PYTHONPATH=experiments "$PYTHON_BIN" \
      experiments/overcooked_v2_experiments/ttappo_v4_actor_adapter/utils/partner_diagnostics.py \
      --d "${run_dir}" \
...
if [[ "${RUN_STAGE1}" == "1" ]]; then
  ...
  run_v4_diagnostics "${smoke_run_dir}" "state_adapt"
fi
...
if [[ "${RUN_STAGE2}" == "1" ]]; then
  ...
  for seed in "${BASE_SEEDS[@]}"; do
    ...
    run_v4_diagnostics "${run_dir}" "state_adapt"
    idx=$((idx + 1))
  done
fi
